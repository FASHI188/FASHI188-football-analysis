...
R=Path(__file__).resolve().parents[2]; BASE='9e55ab07abc13209b545666a5886f54a20a9349d'; N=25; AFTER=245
M=R/'governance/archive/workflows/phase2b-batch07/ARCHIVE_MANIFEST.json'; P=R/'governance/legacy_workflow_migration_plan.json'
def g(*a):
...
def main():
 try:
  ...
  for s,h in m['entries']:
   assert plan[s]['disposition']=='ARCHIVE'; assert not plan[s]['unique_capability'].startswith('FORMAL/GOVERNANCE'); a='governance/archive/workflows/phase2b-batch07/'+Path(s).name; assert bs(BASE,s)==h and not ex('HEAD',s) and ex('HEAD',a) and bs('HEAD',a)==h
 ...
 print('PASS phase2b_batch07'); print('archived_workflow_count=25'); print('active_workflow_count=245'); return 0
