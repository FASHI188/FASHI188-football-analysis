...
This evaluator extracts full-time Correct Score, Total Goals ladder, BTTS and team totals
from that SAME raw response. Only ledger RESULT_SETTLED events are scored.
"""
...
def offer_type(offer):return str((offer.get('betOfferType') or {}).get('englishName') or (offer.get('betOfferType') or {}).get('name') or '')
...
def correct_score_surface(env):
 ...
 ranked=sorted(numeric,key=lambda r:(r['odds'],r['score']))
 complete=bool(rows) and len(valid)==len(rows) and all(r['odds'] for r in rows)
 full_probs=None
 if complete:
  z=sum(1/r['odds'] for r in rows);full_probs={r['name']:(1/r['odds'])/z for r in rows}
 numeric_z=sum(1/r['odds'] for r in numeric)
 numeric_probs={r['score']:(1/r['odds'])/numeric_z for r in numeric} if numeric_z else {}
...
def total_ladder(env):
 ...
 for off in (env.get('payload') or {}).get('betOffers') or []:
  ...
  if abs((ln-math.floor(ln))-0.5)>1e-9:continue
 ...
 lines=sorted(byline)
 complete=all((k+0.5) in byline for k in range(7))
 p=None;max_mono=0.0
 if lines:
  cdfs=[byline[x]['cdf_under'] for x in lines]
  max_mono=max([0.0]+[cdfs[i]-cdfs[i+1] for i in range(len(cdfs)-1)])
 if complete and max_mono<=1e-6:
  c=[byline[k+0.5]['cdf_under'] for k in range(7)];p=[c[0]]+[c[k]-c[k-1] for k in range(1,7)]+[1-c[6]]
  p=[max(0.0,x) for x in p];z=sum(p);p=[x/z for x in p]
...
def main():
 ...
 for mid,pred in sorted(preds.items()):
  ...
  cs=correct_score_surface(env);tl=total_ladder(env);bt=btts(env)
  ...
  if mid in results:
   ...
   if cs and cs['ranked_scores']:
    item['score_top1_hit']=int(cs['ranked_scores'][0]==actual);item['score_top3_hit']=int(actual in cs['ranked_scores'][:3]);item['score_numeric_total_top1_hit']=int(bool(cs['numeric_total_top']) and cs['numeric_total_top'][0]==actual_total)
