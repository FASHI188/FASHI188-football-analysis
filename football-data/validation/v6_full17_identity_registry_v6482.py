...
def collect_recent_evidence(now: datetime) -> dict[str, dict[str, dict[str, Any]]]:
    ...
    for cid in TARGETS:
        ...
        if not items:
            out[cid] = {}
            continue
        # Current weekly captures are clustered. Keep only teams observed close to the
        # freshest evidence so old relegated/stale clubs do not survive a 45-day window.
...
def latest_processed_teams(cid: str) -> tuple[str | None, dict[str, dict[str, Any]]]:
        ...
        try:
            with path.open("r", encoding="utf-8-sig", newline="") as handle:
                for raw in csv.DictReader(handle):
                    ...
                    if home: by_season[season][home] += 1
                    if away: by_season[season][away] += 1
        except Exception:
            continue
    if not by_season:
        return None, {}
    ...
    for team, count in by_season[season].items():
        token = norm(team)
        if token:
            teams[token] = {"canonical_name": team, "observation_count": int(count), "source_path": f"processed/{cid}"}
    return season, teams
...
def load_legacy_aliases() -> dict[str, dict[str, str]]:
    if not LEGACY_ALIASES.exists():
        return {}
    try:
        data = json.loads(LEGACY_ALIASES.read_text(encoding="utf-8"))
        if data.get("fuzzy_matching") is not False:
            return {}
        return {str(cid): {str(k): str(v) for k, v in rows.items()} for cid, rows in (data.get("aliases") or {}).items()}
    except Exception:
        return {}
...
def main() -> int:
    ...
    legacy_aliases = load_legacy_aliases()
    competitions: dict[str, Any] = {}
    available = 0

    for cid in TARGETS:
        recent_rows = recent.get(cid, {})
        latest_season, processed = latest_processed_teams(cid)
        expected_n = int(expected.get(cid) or 0)

        # Prefer the fresh evidence cluster if it is reasonably complete. Otherwise use
        # latest processed-season identity as a fail-closed fallback rather than mixing
        # stale and current teams into one union.
        use_recent = bool(recent_rows) and (expected_n == 0 or len(recent_rows) >= max(8, int(expected_n * 0.80)))
        chosen = recent_rows if use_recent else processed
        source_mode = "RECENT_CURRENT_TEAM_EVIDENCE" if use_recent else "LATEST_PROCESSED_SEASON_FALLBACK"
        ...
        for source, canonical in legacy_aliases.get(cid, {}).items():
            ctoken = norm(canonical)
            if ctoken not in canon_by_norm:
                # Legacy alias can be stale after promotion/relegation; ignore rather than fail the domain.
                continue
            stoken = norm(source)
            if not stoken:
                continue
            prev = exact_aliases.get(stoken)
            if prev is not None and prev != canon_by_norm[ctoken]:
                alias_errors.append(f"ALIAS_COLLISION:{source}:{prev}/{canon_by_norm[ctoken]}")
                continue
            exact_aliases[stoken] = canon_by_norm[ctoken]

        teams = []
        for token, row in sorted(chosen.items()):
            canonical = row["canonical_name"]
            aliases = sorted(source for source, target in exact_aliases.items() if target == canonical)
            teams.append({
        ...
        status = "PASS" if team_count >= 8 and not alias_errors else "PARTIAL_FAIL_CLOSED"
        if status == "PASS":
            available += 1
        competitions[cid] = {
            "competition_id": cid,
    ...
    payload = {
        "schema_version": "V6.48.2-full17-current-team-identity-registry-r1",
        ...
        "policy": "Prefer fresh current-team evidence cluster; fall back to one latest processed season per domain. Exact normalized identity and exact pre-registered provider aliases only. No fuzzy matching or cross-club guessing.",
    ...
    print(json.dumps({
        ...
        "source_modes": {cid: competitions[cid]["identity_source_mode"] for cid in TARGETS},
