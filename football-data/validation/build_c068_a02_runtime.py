...
EXPECTED_IDS_SHA='a91e6274a25c096ff3a70309feab4df245f705001bfc38b8689df4873346b752'
...
def main(matches_zip,events_zip,out):
    ...
    mz=zipfile.ZipFile(matches_zip); ez=zipfile.ZipFile(events_zip); allm=[]
    for n in mz.namelist():
        ...
        for m in json.loads(mz.read(n)):
            mid=int(m['wyId']); key=hashlib.sha256(f'{SEED}|{mid}'.encode()).hexdigest(); allm.append((key,mid,comp,m))
    allm.sort(key=lambda x:x[0]); sel=allm[400:800]; ids=[str(x[1]) for x in sel]
    ...
    manifest=[]
    for rank,(key,mid,comp,m) in enumerate(sel,1): manifest.append({'package_id':'A02','rank':rank,'match_id':mid,'competition_file':comp,'selection_sha256':key,'event_count':len(ev_by[mid])})
    out=Path(out); out.parent.mkdir(parents=True,exist_ok=True)
    with zipfile.ZipFile(out,'w',compression=zipfile.ZIP_DEFLATED) as z:
        z.writestr('PACKAGE.json',json.dumps({'package_id':'A02','series':'A','semantics':'RESEARCH_DEVELOPMENT_ONLY_NOT_PROTECTED','source':'Pappalardo/Wyscout public event dataset','match_count':400,'seed':SEED},indent=2))
    ...
    print(json.dumps({'status':'A02_RUNTIME_REBUILT','matches':400,'ids_sha256':ids_sha,'out':str(out)},indent=2))
