...
sample status, executable PIT evidence, pre-OOF runtime parity and the still-separate
competition/season promotion gate. No formal weight is changed.
"""
...
def main()->int:
    ...
    for cid,spec in TARGETS.items():
        ...
        checks={
          ...
          "pre_oof_dynamic_strength_formula_parity_passed":pre_oof_smoke.get("status")=="PASS" and bool(pre_oof_checks.get(spec["pre_oof_smoke_check"])),
          "pre_oof_dynamic_strength_dormant_without_receipt":pre_oof_smoke.get("status")=="PASS" and bool(pre_oof_checks.get("no_promotion_receipt_keeps_matrix_unchanged")) and bool(pre_oof_checks.get("no_promotion_receipt_status_not_enabled")),
          "actionable_runtime_dynamic_strength_effect_wired":pre_oof_smoke.get("status")=="PASS",
