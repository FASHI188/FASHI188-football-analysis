...
current-season sample status, executable PIT evidence contract and runtime wiring.
No formal weight is changed.
"""
...
TARGETS={
 "ESP_LaLiga":{"target_season":"2026/27","final_path":ROOT/"manifests"/"dynamic_strength_final_chain_replay_v470"/"ESP_LaLiga.json","expected_final_status":"FINAL_CHAIN_LIVE_INPUT_REVIEW_REQUIRED","mode":"full_dynamic_strength"},
 "NED_Eredivisie":{"target_season":"2026/27","final_path":ROOT/"manifests"/"dynamic_strength_allocation_only_final_chain_v470"/"NED_Eredivisie.json","expected_final_status":"ALLOCATION_ONLY_FINAL_CHAIN_LIVE_INPUT_REVIEW_REQUIRED","mode":"allocation_only_preserve_direct_total"},
}
def load(path):return json.loads(path.read_text(encoding="utf-8"))
def main()->int:
    registry=load(REGISTRY);reg={r["competition_id"]:r for r in registry["competitions"]};selection=load(SELECTION) if SELECTION.exists() else {};smoke=load(LIVE_SMOKE) if LIVE_SMOKE.exists() else {}
    reports={}
    for cid,spec in TARGETS.items():
        ...
        checks={
          ...
          "live_input_contract_supports_competition":cid=="ESP_LaLiga",
