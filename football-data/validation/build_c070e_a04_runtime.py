...
def main(matches_zip: str, events_zip: str, out: str) -> None:
    ...
    if len(selected) != EXPECTED_COUNT:
        raise RuntimeError(f"A04 selection count {len(selected)} != {EXPECTED_COUNT}")
    ...
    if set(events_by_match) != wanted:
        ...
        raise RuntimeError(
            f"A04 event coverage={len(events_by_match)} expected={EXPECTED_COUNT}; missing={missing[:10]}"
        )
            ...
            json.dumps(
                {
                    "package_id": "A04",
                    "series": "A",
                    "semantics": "FRESH_RESEARCH_CALIBRATION_CONFIRMATION_NOT_PROTECTED",
