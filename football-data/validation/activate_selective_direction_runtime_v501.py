#!/usr/bin/env python3
"""Build or verify the V5.0.1-bound LaLiga selective-direction activation."""
...
def build_payload() -> dict:
    ...
    checks = {
        ...
        "v500_promotion_governance_activated": str(v500.get("status") or "").startswith(
            "FORMALLY_ACTIVATED"
        ),
        ...
        "runner_final_call_wired": (
            "return apply_selective_direction_gate(context, governed)" in runner_text
        ),
    }
    ...
    return {
        ...
        "activation_order": (
            "after_final_probability_matrix_market_coordination_total_diagnostics_and_formal_governance"
        ),
        "selected_threshold": 0.30,
        "formal_probability_weight": 0,
        "probability_mutation": False,
        "direction_semantics": (
            "ALLOW_FINAL_1X2_TOP1_IF_TOP1_MINUS_TOP2_GAP_GTE_0.30_ELSE_ABSTAIN"
        ),
        "bound_git_blob_sha": {
            key: git_blob_sha(path) for key, path in PATHS.items()
        },
        "bound_paths": {
            key: path.relative_to(REPO_ROOT).as_posix() for key, path in PATHS.items()
        },
        "formal_rule_binding": {
            "version": "V5.0.1",
        ...
        "promotion_rule_binding": {
            "version": v500.get("formal_rule_version"),
            "promotion_receipt_status": promotion.get("status"),
        },
        "checks": checks,
        "policy": (
            "Fail closed on any bound path, Git blob, CURRENT, competition or season mismatch. "
            "This gate changes direction eligibility only and never mutates probabilities."
        ),
    }
