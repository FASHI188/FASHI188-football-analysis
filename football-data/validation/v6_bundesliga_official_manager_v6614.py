#!/usr/bin/env python3
"""V6.6.14 ingest current managers for deficient-roster Bundesliga teams from Bundesliga.com.
...
UA='football-v6.6.14-bundesliga-manager/1.0'
TEAMS={
 ...
 for i,v in enumerate(t):
  if v.strip().casefold()=='head coach':
   for nxt in t[i+1:i+6]:
    n=nxt.strip()
    if n and n.casefold() not in {'stats','news','squad','head coach','advertisement'} and len(n)<=80:
     candidates.append(n);break
 # collapse exact repeats only; ambiguous distinct names fail closed
 uniq=[]
 for x in candidates:
...
def prior_managers():
 ...
 for p in OUTDIR.glob('*.json'):
  ...
  for r in rows:
   ...
   try:ts=datetime.fromisoformat(str(r.get('observed_at_utc') or '').replace('Z','+00:00'))
   except Exception:continue
   if team not in latest or ts>latest[team][0]:latest[team]=(ts,str(coach))
 return {k:v[1] for k,v in latest.items()}
def norm(s):return re.sub(r'\s+',' ',str(s)).strip().casefold()
def main():
 observed=datetime.now(timezone.utc).replace(microsecond=0);prior=prior_managers();records=[];audit=[]
 for team,slug in TEAMS.items():
  url=BASE.format(slug);row={'team_name':team,'source_url':url,'status':'FAIL_CLOSED'}
  try:
   markup,source_ts=fetch(url);managers,t=parse_manager(markup);incomplete=any(v.strip().casefold()=='not complete' for v in t);row.update({'manager_candidates':managers,'page_explicitly_squad_not_complete':incomplete})
   if len(managers)==1:
 ...
 OUTDIR.mkdir(parents=True,exist_ok=True);STATUS.parent.mkdir(parents=True,exist_ok=True);stamp=observed.strftime('%Y%m%dT%H%M%SZ');path=OUTDIR/f'bundesliga_managers__{stamp}.json'
 if records:path.write_text(json.dumps({'schema_version':'V6.6.14-bundesliga-manager-weekly-aggregate-r1','observed_at_utc':observed.isoformat(),'records':records,'governance':{'official_manager_only':True,'incomplete_squad_not_used_for_roster':True,'formal_probability_use':False}},ensure_ascii=False,indent=2),encoding='utf-8')
 payload={'schema_version':'V6.6.14-bundesliga-manager-status-r1','generated_at_utc':observed.isoformat(),'status':'PASS_COMPLETE' if len(records)==len(TEAMS) else 'WARN_PARTIAL' if records else 'FAIL_NO_VALID_MANAGER','target_count':len(TEAMS),'valid_manager_count':len(records),'evidence_path':str(path.relative_to(ROOT)) if records else None,'audit':audit,'governance':{'official_source_only':True,'manager_only':True,'bundesliga_squad_not_complete_is_hard_exclusion_from_strict_roster':True,'research_context_only':True,'formal_probability_change':False,'formal_weight_change':False}}
