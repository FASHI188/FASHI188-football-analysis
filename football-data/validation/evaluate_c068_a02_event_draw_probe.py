...
IDS_SHA='a91e6274a25c096ff3a70309feab4df245f705001bfc38b8689df4873346b752'
...
def load(p):
 ...
 if pkg['package_id']!='A02' or pkg['match_count']!=400 or sha!=IDS_SHA: raise RuntimeError('A02 identity mismatch')
...
def run(a02,out):
 cf,ms,ev=load(a02);D=build(cf,ms,ev);D=D[(D.hn>=2)&(D.an>=2)].reset_index(drop=True);N=len(D)
 if N!=249: raise RuntimeError(f'eligible={N}')
 s1=N//2;s2=(3*N)//4;fs={};YY=[];BB=[];CC=[]
 for name,s,e in [('fold_1',s1,s2),('fold_2',s2,N)]:
 ...
 b,c=met(np.array(YY),np.vstack(BB)),met(np.array(YY),np.vstack(CC));delta={k:c[k]-b[k] for k in ['accuracy','macro_f1','log_loss','brier','rps','draw_calls','draw_hits','draw_f1']};stable=delta['log_loss']<0 and delta['brier']<=0 and delta['rps']<=0 and c['draw_hits']>b['draw_hits'];x={'schema_version':'C068_A02_EVENT_DRAW_PROBE_R1','status':'POSTVIEW_A02_ONLY_DEVELOPMENT_PROBE_COMPLETE','verdict':'A02_EVENT_HISTORY_DEVELOPMENT_SIGNAL' if stable else 'A02_EVENT_HISTORY_STABLE_DRAW_INCREMENT_NOT_ESTABLISHED','source':{'package':'A02','matches':400,'ids_sha256':IDS_SHA,'semantics':'RESEARCH_DEVELOPMENT_ONLY_NOT_PROTECTED'},'contract':{'same_match_events_used_for_prediction':False,'history_update':'after UTC date batch','min_prior_package_matches_each_team':2,'eligible_rows':N,'features':FEAT,'C':C,'class_weight':None,'blend':W,'min_pD':MINPD,'max_abs_HA_gap':MAXGAP,'min_residual':MINRES,'natural_argmax_only':True},'folds':fs,'pooled':{'rows':len(YY),'baseline':b,'candidate':c,'delta':delta},'boundary':{'other_A_packages_used':False,'protected_samples_used':False,'formal_weight':0,'scientific_pass':False,'confirmation_pass':False,'post_view_development_only':True}};Path(out).write_text(json.dumps(x,ensure_ascii=False,indent=2));print(json.dumps(x,ensure_ascii=False,indent=2))
if __name__=='__main__':
 import argparse
 a=argparse.ArgumentParser();a.add_argument('--a02',required=True);a.add_argument('--out',required=True);q=a.parse_args();run(Path(q.a02),q.out)
