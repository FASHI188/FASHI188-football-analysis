#!/usr/bin/env python3
"""Fixed 100-match exploration on the existing neutral 1X2 benchmark.

The 100 identities are selected from the market-complete pool before any result
label is parsed. All market-complete rows are then processed prequentially in
strict date order, with same-day predictions frozen before updates.

Research only: formal_weight=0; no provider request, external collection,
formal promotion, current-match output, unified matrix, exact score or EV.
"""
...
def identity(row: dict[str, Any]) -> str:
    ...
    if not all(parts):
        raise StudyError(f"incomplete identity: {parts}")
    return "|".join(parts)


def normalize(probabilities: dict[str, Any]) -> dict[str, float]:
    try:
        return base.normalized_probs(probabilities)
    except Exception as exc:
        raise StudyError(str(exc)) from exc


def prepare_market_pool(
    benchmark: dict[str, Any],
) -> tuple[list[dict[str, Any]], dict[str, int], int]:
    """Build the eligible pool using only pre-result fields."""
    ...
    exclusions = {
        "row_not_object": 0,
        "missing_market_probabilities": 0,
        "invalid_market_probabilities": 0,
    }
    seen: set[str] = set()
    ...
    for index, raw in enumerate(raw_rows):
        ...
        key = identity(raw)
        if key in seen:
            raise StudyError(f"duplicate benchmark identity: {key}")
        seen.add(key)

        try:
            date = datetime.fromisoformat(str(raw.get("date") or "").replace("Z", "+00:00"))
        except ValueError as exc:
            raise StudyError(f"invalid date at row {index}") from exc
        if date.tzinfo is None:
            raise StudyError(f"naive date at row {index}")

        market = raw.get("market")
        probabilities = market.get("probabilities") if isinstance(market, dict) else None
        if not isinstance(probabilities, dict) or any(
            direction not in probabilities for direction in DIRECTIONS
        ):
            exclusions["missing_market_probabilities"] += 1
            continue
        try:
            normalized = normalize(probabilities)
        except StudyError:
            exclusions["invalid_market_probabilities"] += 1
            continue

        eligible.append(
            {
                **raw,
                "market": {**market, "probabilities": normalized},
                "_identity": key,
            }
        )
    ...
    if len(eligible) + sum(exclusions.values()) != len(raw_rows):
        raise StudyError("pre-result pool accounting mismatch")
    return eligible, exclusions, len(raw_rows)


def attach_labels_after_selection(
    market_pool: list[dict[str, Any]],
) -> list[dict[str, Any]]:
    """Parse every eligible label only after the 100 identities are frozen."""
    labeled: list[dict[str, Any]] = []
    for index, row in enumerate(market_pool):
        try:
            actual = base.actual_direction(row)
        except Exception as exc:
            raise StudyError(
                f"invalid result label in market-complete pool at row {index}"
            ) from exc
        labeled.append({**row, "actual": actual})
    return labeled


def quota_by_competition(
    ...
    exact = {
        competition: target * count / total
        for competition, count in counts.items()
    }
    quota = {
        competition: min(counts[competition], int(math.floor(value)))
        for competition, value in exact.items()
    }
    remaining = target - sum(quota.values())
    order = sorted(
        counts,
        key=lambda competition: (
            -(exact[competition] - quota[competition]),
            competition,
        ),
    )
    while remaining:
...
def fixed_sample(
    ...
    for row in rows:
        grouped[str(row["competition_id"])].append(row)
    ...
    for competition in sorted(grouped):
        ranked = sorted(
            ...
            key=lambda row: (
                hashlib.sha256(
                    f"{seed}|{row['_identity']}".encode("utf-8")
                ).hexdigest(),
                row["_identity"],
            ),
        )
        selected.update(
            row["_identity"] for row in ranked[: quota[competition]]
        )
...
def normalize_output(probabilities: dict[str, float]) -> dict[str, float]:
    values = {
        direction: max(EPS, float(probabilities[direction]))
...
def linear_pool(
    ...
    return normalize_output(
        {
            direction: (1.0 - weight) * market[direction]
            + weight * residual[direction]
            for direction in DIRECTIONS
        }
    )
...
def log_pool(
    market: dict[str, float], residual: dict[str, float]
) -> dict[str, float]:
    return normalize_output(
        {
            direction: math.sqrt(market[direction] * residual[direction])
            for direction in DIRECTIONS
        }
    )
...
def draw_only(
    ...
    draw = (
        (market["draw"] + residual["draw"]) / 2.0
        if half
        else residual["draw"]
    )
    draw = min(1.0 - EPS, max(EPS, draw))
    non_draw = market["home"] + market["away"]
    if non_draw <= 0:
        raise StudyError("invalid market non-draw mass")
    home_share = market["home"] / non_draw
    return normalize_output(
        {
            "home": (1.0 - draw) * home_share,
            "draw": draw,
            "away": (1.0 - draw) * (1.0 - home_share),
        }
    )
...
def prequential_predictions(
    rows: list[dict[str, Any]], model_config: dict[str, Any]
) -> list[dict[str, Any]]:
    model = base.MarketResidualModel(model_config.get("market_residual") or {})
    ...
    for day in sorted(by_day):
        ...
        for row in by_day[day]:
            ...
            market = normalize_output(features["market"])
            residual = normalize_output(model.predict(features))
            output.append(
                {
...
def pick(probability: dict[str, float]) -> str:
    priority = {"home": 2, "draw": 1, "away": 0}
    return max(
        DIRECTIONS,
        key=lambda direction: (probability[direction], priority[direction]),
    )


def row_components(
    row: dict[str, Any], candidate: str
) -> dict[str, float]:
    probability = row[candidate]
    actual = row["actual"]
    target = {
        direction: 1.0 if direction == actual else 0.0
        for direction in DIRECTIONS
    }
    return {
        "accuracy": float(pick(probability) == actual),
        "log_loss": -math.log(max(EPS, probability[actual])),
        "brier": sum(
            (probability[direction] - target[direction]) ** 2
            for direction in DIRECTIONS
        ),
        "rps": (
...
def summarize(
    ...
    confusion = {
        actual: {predicted: 0 for predicted in DIRECTIONS}
    ...
    draw_precision = (
        true_draw / predicted_counts["draw"]
        if predicted_counts["draw"]
        else 0.0
    )
    draw_recall = (
        true_draw / actual_counts["draw"] if actual_counts["draw"] else 0.0
    )
    draw_f1 = (
    ...
    non_draw_hits = sum(
        pick(row[candidate]) == row["actual"] for row in non_draw_rows
    )

    return {
        "rows": len(rows),
        "hits": int(sum(value["accuracy"] for value in components)),
        "accuracy": sum(value["accuracy"] for value in components) / len(rows),
        "log_loss": sum(value["log_loss"] for value in components) / len(rows),
        "brier": sum(value["brier"] for value in components) / len(rows),
        "rps": sum(value["rps"] for value in components) / len(rows),
        "actual_counts": {
            direction: int(actual_counts[direction])
            for direction in DIRECTIONS
        },
        "predicted_counts": {
            direction: int(predicted_counts[direction])
            for direction in DIRECTIONS
        },
        ...
        "draw_f1": draw_f1,
        "non_draw_accuracy": (
            non_draw_hits / len(non_draw_rows) if non_draw_rows else 0.0
        ),
        "mean_confidence": sum(
            max(row[candidate].values()) for row in rows
        )
        / len(rows),
        "confusion": confusion,
    }
...
def confidence_summaries(
    ...
    ordered = sorted(
        rows,
        key=lambda row: (-max(row[candidate].values()), row["identity"]),
    )
    output: dict[str, Any] = {}
    for coverage in coverages:
        count = max(
            1, min(len(rows), int(round(len(rows) * float(coverage))))
        )
        output[f"{int(round(float(coverage) * 100))}pct"] = summarize(
...
def paired_bootstrap(
    ...
    interval = [
        float(value) for value in cfg["evaluation"]["interval"]
    ]
    base_seed = int(cfg["evaluation"]["paired_bootstrap_seed"])
    output: dict[str, Any] = {}
    ...
    for metric_offset, metric in enumerate(
        ...
        delta = [
            challenger[index][metric] - baseline[index][metric]
            for index in range(len(rows))
        ]
        rng = random.Random(
            base_seed + candidate_offset * 100 + metric_offset
        )
        draws = [
            sum(delta[rng.randrange(len(rows))] for _ in range(len(rows)))
            / len(rows)
...
def run(config_path: Path, out_dir: Path) -> dict[str, Any]:
    ...
    model_config_path = ROOT / str(cfg["source_market_residual_config"])
    benchmark = load_json(benchmark_path)
    model_config = load_json(model_config_path)

    market_pool, exclusions, source_rows = prepare_market_pool(benchmark)
    expected_source_rows = int(cfg["sample_contract"]["source_rows"])
    if source_rows != expected_source_rows:
        raise StudyError(
            f"source row mismatch: {source_rows} != {expected_source_rows}"
        )
    minimum = int(cfg["sample_contract"]["eligible_pool_minimum"])
    if len(market_pool) < minimum:
        raise StudyError(
            f"market-complete pool too small: {len(market_pool)} < {minimum}"
        )
    ...
    pool_identity_sha = hashlib.sha256(
        "\n".join(sorted(row["_identity"] for row in market_pool)).encode(
            "utf-8"
        )
    ).hexdigest()
    selected_identity_sha = hashlib.sha256(
        "\n".join(sorted(selected)).encode("utf-8")
    ).hexdigest()

    # Label access begins only after the selected identity set is immutable.
    labeled_pool = attach_labels_after_selection(market_pool)
    all_predictions = prequential_predictions(labeled_pool, model_config)
    sample = sorted(
        [
            row
    ...
    if len(sample) != 100 or len(
        {row["identity"] for row in sample}
    ) != 100:
    ...
    for candidate in candidates[1:]:
        ...
        if (
            value["accuracy"] - market["accuracy"]
            > float(gate["accuracy_delta_minimum"])
            and value["log_loss"] - market["log_loss"]
            <= float(gate["log_loss_delta_maximum"])
            and value["brier"] - market["brier"]
            <= float(gate["brier_delta_maximum"])
        ):
    ...
    result = {
        "schema_version": "v511_fixed100_market_residual_r34_status.2",
        "status": status,
        ...
        "source": {
            ...
            "market_complete_eligible_rows": len(market_pool),
            "pre_result_exclusions": exclusions,
            "market_complete_pool_identity_sha256": pool_identity_sha,
            "source_market_residual_config": str(
                model_config_path.relative_to(ROOT)
            ),
        ...
        "sample": {
            ...
            "selection_frozen_before_any_label_parsing": True,
            "quota_by_competition": quota,
            "identity_sha256": selected_identity_sha,
            "actual_counts_revealed_only_after_selection": metrics["market"][
                "actual_counts"
            ],
            "no_resampling_after_result": True,
        },
        "execution": {
            "prequential_predictions_over_all_market_complete_rows": True,
            "prequential_prediction_rows": len(all_predictions),
        ...
        "ruling": {
            ...
            "interpretation": (
                "At least one fixed candidate improved full-coverage point "
                "accuracy without worsening point Log Loss or Brier on this "
                "viewed 100-match exploratory sample. Independent "
                "confirmation remains required."
                if promising
                else
                "No fixed candidate improved full-coverage point accuracy "
                "while preserving point Log Loss and Brier on this viewed "
                "100-match exploratory sample."
            ),
        },
        ...
        summary_rows.append(
            {
                ...
                "accuracy_delta_vs_market": (
                    value["accuracy"] - market["accuracy"]
                ),
                "log_loss": value["log_loss"],
                "log_loss_delta_vs_market": (
                    value["log_loss"] - market["log_loss"]
                ),
                "brier": value["brier"],
                "brier_delta_vs_market": (
                    value["brier"] - market["brier"]
                ),
                "rps": value["rps"],
    ...
    manifest = {
        "schema_version": "v511_fixed100_market_residual_r34_artifact.2",
        "files": {},
    }
        ...
        json.dumps(
            {
                ...
                "promising_point_candidates": result[
                    "promising_point_candidates"
                ],
