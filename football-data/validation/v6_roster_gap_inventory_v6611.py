...
roster and therefore stays in the strict-current gap queue.
"""
...
def active_map()->dict:
 ...
 for p in ACTIVE.glob('*.json') if ACTIVE.exists() else []:
  ...
  for r in rows:
   ...
   try:obs=datetime.fromisoformat(str(r.get('observed_at_utc') or '').replace('Z','+00:00'))
   except Exception:continue
...
def main()->int:
 team=load(TEAM);current=load(CURRENT);prov=load(PROV);effective=load(EFFECTIVE);latest=[r for r in team.get('latest') or [] if isinstance(r,dict)]
 if not latest:raise SystemExit('weekly team latest inventory missing')
 additions={key(r.get('competition_id'),r.get('resolved_team_name')) for r in current.get('matched_overlays') or [] if r.get('strict_roster_addition') is True};active=active_map();prov_map={}
 ...
 payload={'schema_version':'V6.6.12-roster-gap-inventory-r2','generated_at_utc':datetime.now(timezone.utc).replace(microsecond=0).isoformat(),'status':'PASS' if consistency and conservation else 'FAIL_CONTEXT_STATE_MISMATCH','formal_current_version':'V5.0.1','team_count':len(latest),'effective_strict_current_count':strict,'unresolved_strict_current_gap_count':len(gaps),'priority_counts':{k:int(counts.get(k,0)) for k in ('A_NO_CURRENT_CONTEXT','B_HAS_PROVISIONAL_ONLY','C_HAS_ACTIVE_MATCH_POOL')},'competition_gap_counts':dict(sorted(by_comp.items())),'gaps':gaps,'consistency_check':{'effective_states':states,'inventory_expected_priority_counts':expected,'inventory_matches_effective_states':consistency,'strict_plus_gap_conservation':conservation},'governance':{'derived_from_validated_receipts_only':True,'active_match_pool_never_counts_as_strict':True,'provisional_never_counts_as_strict':True,'no_player_list_union_across_provider_groups':True,'research_context_only':True,'formal_probability_change':False,'formal_weight_change':False}}
