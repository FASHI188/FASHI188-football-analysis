...
Repair order is source-preserving: first retry the exact primary ESPN roster surface because
preseason feeds can change within minutes; only if it is still sub-18 may TheSportsDB be used as
a tier-3 fallback. A replacement is written only when a candidate list is larger than the stored
list. Lists are never merged across sources. Existing injury/transaction/depth evidence is kept.
"""
...
TSD='https://www.thesportsdb.com/api/v1/json/123';ESPN='https://site.api.espn.com/apis/site/v2/sports/soccer';UA='football-v6.6.4-roster-repair/2.0';MIN_PLAYERS=18;REQUEST_INTERVAL_SECONDS=2.05;_last_tsd=0.0
...
def espn_players(snapshot:dict[str,Any])->tuple[list[dict[str,Any]],str|None]:
 ids=snapshot.get('provider_ids') or {};league=str(ids.get('espn_league_slug') or '');tid=str(ids.get('espn_team_id') or '')
 if not league or not tid:return [],None
 url=f'{ESPN}/{league}/teams/{tid}/roster';payload=get_json(url);athletes=payload.get('athletes') or payload.get('players') or [];out=[];seen=set()
 for group in athletes if isinstance(athletes,list) else []:
  ...
  for player in seq:
   ...
   if isinstance(pos,dict):positions=[x for x in (pos.get('displayName'),pos.get('abbreviation')) if x]
   elif pos:positions=[str(pos)]
   out.append({'player_id':str(player.get('id') or player.get('uid') or '') or None,'player_name':name,'positions':positions,'age':player.get('age'),'shirt_number':player.get('jersey') or player.get('jerseyNumber'),'squad_status':(player.get('status') or {}).get('name') if isinstance(player.get('status'),dict) else player.get('status'),'roster_source':'ESPN primary roster retry'})
 return out,url
...
def make_overlay(snapshot:dict[str,Any],players:list[dict[str,Any]],generated:datetime,source_name:str,source_tier:str,source_url:str,reason:str,before:int,team:dict[str,Any]|None=None)->dict[str,Any]:
 repaired=json.loads(json.dumps(snapshot));repaired['schema_version']='V6.6.4-roster-repair-overlay-r2';repaired['observed_at_utc']=generated.isoformat();repaired['players']=players
 ...
 g=repaired.setdefault('governance',{});g['sub18_repair_overlay']=True;g['primary_availability_transactions_depth_preserved']=True;g['formal_probability_use']=False;g['no_cross_source_player_list_merge']=True
 return repaired
def main()->int:
 latest=latest_aggregate();generated=now()
 if latest is None:
  payload={'schema_version':'V6.6.4-roster-repair-status-r2','generated_at_utc':generated.isoformat(),'status':'NO_WEEKLY_AGGREGATE','repaired':0};OUT.parent.mkdir(parents=True,exist_ok=True);OUT.write_text(json.dumps(payload,ensure_ascii=False,indent=2),encoding='utf-8');print(json.dumps(payload,ensure_ascii=False,indent=2));return 0
 ...
 for snapshot in deficient:
  ...
  try:
   ep,eu=espn_players(snapshot);record['espn_retry_players']=len(ep)
   if len(ep)>before and len(ep)>=MIN_PLAYERS:
    overlays.append(make_overlay(snapshot,ep,generated,'ESPN public site API','tier_2',eu or '', 'PRIMARY_SAME_SOURCE_RETRY_RECOVERED',before));record['status']='REPAIRED_TO_STRICT_ROSTER_ESPN_RETRY';attempts.append(record);continue
   tp,team,tu=tsd_fallback(snapshot);record['fallback_players']=len(tp)
   best_count=max(before,len(ep))
   if len(tp)>best_count:
    overlays.append(make_overlay(snapshot,tp,generated,'TheSportsDB free API','tier_3_fallback',tu or '', 'PRIMARY_RETRY_STILL_SUB18',before,team));record['status']='REPAIRED_TO_STRICT_ROSTER_TSD' if len(tp)>=MIN_PLAYERS else 'IMPROVED_BUT_STILL_SUB18'
   elif len(ep)>before:
    overlays.append(make_overlay(snapshot,ep,generated,'ESPN public site API','tier_2',eu or '', 'PRIMARY_SAME_SOURCE_RETRY_IMPROVED_BUT_SUB18',before));record['status']='ESPN_RETRY_IMPROVED_BUT_STILL_SUB18'
 ...
 overlay_path=None
 if overlays:
  stamp=generated.strftime('%Y%m%dT%H%M%SZ');overlay_path=EVIDENCE/f'weekly_roster_repair__{stamp}.json';overlay={'schema_version':'V6.6.4-weekly-roster-repair-aggregate-r2','observed_at_utc':generated.isoformat(),'source_weekly_aggregate':str(source_path.relative_to(ROOT)),'snapshot_count':len(overlays),'snapshots':overlays,'governance':{'append_only_overlay':True,'formal_probability_use':False,'same_source_retry_precedes_tier3':True,'no_cross_source_player_list_merge':True}};overlay_path.write_text(json.dumps(overlay,ensure_ascii=False,indent=2),encoding='utf-8')
 ...
 payload={'schema_version':'V6.6.4-roster-repair-status-r2','generated_at_utc':generated.isoformat(),'status':'PASS' if not errors else 'WARN','source_weekly_aggregate':str(source_path.relative_to(ROOT)),'total_team_snapshots':len(snapshots),'sub18_before_repair':len(deficient),'repair_attempt_count':len(attempts),'strict_repairs_created':strict,'strict_espn_retries':sum(1 for x in attempts if x['status']=='REPAIRED_TO_STRICT_ROSTER_ESPN_RETRY'),'strict_tsd_repairs':sum(1 for x in attempts if x['status']=='REPAIRED_TO_STRICT_ROSTER_TSD'),'overlays_created':len(overlays),'overlay_path':str(overlay_path.relative_to(ROOT)) if overlay_path else None,'attempts':attempts,'governance':{'research_context_only':True,'no_formal_probability_change':True,'same_source_retry_first':True,'no_cross_source_player_list_merge':True}}
