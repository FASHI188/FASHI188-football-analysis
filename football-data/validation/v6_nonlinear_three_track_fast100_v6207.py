...
non-inferior. Exact score keeps its own market-only T_score layer and independently
selects the conditional P(H|T,X) feature/model family by validation exact-score LogLoss.
The T_score feature space and conditional-allocation feature space are separate.
"""
...
def cond_score_p(r,score_total_model,cm,q7,shot_names,comps,conditional_process):
    # T_score is ALWAYS evaluated in its own market-only feature space.
    ...
    for proc in (False,True):
        for C in base.CS:
            for alpha in (0.0,0.25,0.5):
                ...
                q=base.score_metrics(valid,lambda r,cm=cm,q7=q7,proc=proc:cond_score_p(r,score_total_model,cm,q7,shot_names,comps,proc));q.update({'family':'logistic','process':proc,'C':C,'alpha':alpha});board.append(q)
    for cfg in HGB_CFGS:
        for alpha in BALANCES:
...
def score_with_chosen(r,score_total_model,cm,q7,proc,family,shot_names,comps):
    return cond_score_p(r,score_total_model,cm,q7,shot_names,comps,proc)
...
def main():
    ...
    _,lin_cs=base.select_cond(select_train,valid,score_total_select,shot_names,comps,False);lin_cm,lin_q7=base.fit_cond(final_train,lin_cs['C'],lin_cs['alpha'],shot_names,comps,False);lin_scr=base.score_metrics(sample,lambda r:cond_score_p(r,score_total_model,lin_cm,lin_q7,shot_names,comps,False))
