#!/usr/bin/env python3
"""V6.12.9b research-only Fast100: recent international-duty load in expected XI.

Expected XI is built only from same-season earlier club lineups. International duty is
an actual appearance in a national_team_competition strictly before the target club match.
The Fast100 exposure pool uses a 1-10 day window to obtain 100 affected 2025/26 matches;
1-5 day high-load features are retained separately inside the same test.
"""
...
def load_international_appearances():
    ...
    atext,abytes=fetch(APPS,240);ard=csv.DictReader(io.StringIO(atext));by_player=defaultdict(list);app_rows=0;intl_app_rows=0;minutes_positive=0;cols=list(ard.fieldnames or [])
    for r in ard:
...
def build_rows(intl):
    ...
    for r in matches:
        cid=r['competition_id'];season=r['season'];ds=str(r['date'])[:10];target=date.fromisoformat(ds);hk=(cid,season,r['home']);ak=(cid,season,r['away'])
        ...
        if hp is not None and ap is not None:
            ...
            for w in (5,10):
                fp=vals[f'home_players_{w}'] if fav=='home' else vals[f'away_players_{w}'] if fav=='away' else 0
                dp=vals[f'away_players_{w}'] if fav=='home' else vals[f'home_players_{w}'] if fav=='away' else 0
                fm=vals[f'home_minutes_{w}'] if fav=='home' else vals[f'away_minutes_{w}'] if fav=='away' else 0.0
                dm=vals[f'away_minutes_{w}'] if fav=='home' else vals[f'home_minutes_{w}'] if fav=='away' else 0.0
                vals[f'fav_players_{w}']=fp;vals[f'dog_players_{w}']=dp;vals[f'player_diff_{w}']=fp-dp;vals[f'fav_minutes_{w}']=fm;vals[f'dog_minutes_{w}']=dm;vals[f'minute_diff_{w}']=fm-dm
...
def main():
    intl,source=load_international_appearances();rows=build_rows(intl);latest=[r for r in rows if r['season']=='2025/26'];affected10=[r for r in latest if r['home_players_10'] or r['away_players_10']];affected5=[r for r in latest if r['home_players_5'] or r['away_players_5']]
    if len(affected10)<100:raise RuntimeError(f'insufficient 1-10d international-duty affected matches: {len(affected10)}; 1-5d={len(affected5)}')
    test=affected10[-100:]
    payload={'schema_version':'V6.12.9b-international-duty-fast100-r1','generated_at_utc':datetime.now(timezone.utc).replace(microsecond=0).isoformat(),'status':'PASS','formal_current_version':'V5.0.1','classification':'RETROSPECTIVE_RESEARCH_ONLY_STRICTLY_PRIOR_INTERNATIONAL_APPEARANCES','governance':{'expected_xi_same_season_prior_only':True,'exposure_pool_1_to_10_days_prior':True,'high_load_1_to_5_days_retained':True,'target_actual_xi_excluded':True,'exposure_selection_outcome_independent':True,'test_matches':100,'formal_probability_change':False,'formal_weight_change':False,'current_rule_change':False},'source':source,'sample':{'feature_rows_2025_26':len(latest),'affected_1_5d_2025_26':len(affected5),'affected_1_10d_2025_26':len(affected10),'test_first':test[0]['date'],'test_last':test[-1]['date']},'test':{'all_affected10':stat(test),'any_1_5d':stat(test,lambda r:r['home_players_5'] or r['away_players_5']),'favorite_3plus_5d':stat(test,lambda r:r['fav_players_5']>=3),'favorite_more_by2_5d':stat(test,lambda r:r['player_diff_5']>=2),'favorite_3plus_10d':stat(test,lambda r:r['fav_players_10']>=3),'favorite_more_by2_10d':stat(test,lambda r:r['player_diff_10']>=2),'p_ge_0.58':stat(test,lambda r:max(r['opening'])>=0.58),'p_ge_0.58_exclude_fav3_5d':stat(test,lambda r:max(r['opening'])>=0.58 and r['fav_players_5']<3),'p_ge_0.58_exclude_fav3_10d':stat(test,lambda r:max(r['opening'])>=0.58 and r['fav_players_10']<3),'p_ge_0.60':stat(test,lambda r:max(r['opening'])>=0.60),'p_ge_0.60_exclude_fav3_5d':stat(test,lambda r:max(r['opening'])>=0.60 and r['fav_players_5']<3)}}
