...
def build(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for row in rows:
        ...
        all_surface_times.extend((one_times[-1], ah_times[-1], ou_times[-1]))
    global_skew = (max(all_surface_times) - min(all_surface_times)).total_seconds()
    ...
    observed_at = max(all_surface_times)
    ...
    payload = {
        ...
        "consensus_surface_observed_at_utc": {
        ...
        "surface_consensus_eligibility": {
            ...
            "over_under_2_5": ou is not None and abs(float(ou["line"]) - 2.5) <= 1e-9,
        },
