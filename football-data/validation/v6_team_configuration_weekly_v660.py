#!/usr/bin/env python3
"""V6.6.9 weekly team-context validator.
...
FC/SC/CF/AFC may be stripped, but fuzzy cross-club substitution is prohibited.
"""
...
def main() -> int:
    ...
    for (cid,team),(ts,snapshot,filename) in sorted(latest.items()):
        ...
        manager_overlay=latest_manager.get((cid,team)); manager_record=manager_overlay[1] if manager_overlay else None; coach=(manager_record or {}).get("head_coach") or machine_coach; manager_ok=bool(manager_record) or (bool(machine_coach) and strong); change=(manager_record or {}).get("manager_change") or {}; manager_change_ok=manager_ok and change.get("status") in {"UNCHANGED","CHANGED_CONFIRMED","INTERIM_CONFIRMED"}; complete=availability_ok and transaction_ok and manager_ok
