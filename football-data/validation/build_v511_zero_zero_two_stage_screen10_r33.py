#!/usr/bin/env python3
"""Build the fixed R33 two-stage 0-0 blind packet.

All identities revealed by R29, R30 and R32 are deterministically reconstructed and
excluded. Public output contains only fixture and market inputs; roles and scores are
...
import build_v511_zero_zero_screen10_r32 as r32
...
DEFAULT_R32_CONFIG = ROOT / "config" / "v511_zero_zero_screen10_r32.json"
DEFAULT_STATUS = ROOT / "manifests" / "v511_zero_zero_two_stage_screen10_r33_status.json"
DEFAULT_PUBLIC = ROOT / "manifests" / "v511_zero_zero_two_stage_screen10_r33_public.json"
DEFAULT_PUBLIC_CSV = ROOT / "manifests" / "v511_zero_zero_two_stage_screen10_r33_public.csv"
DEFAULT_HIDDEN = ROOT / "manifests" / "v511_zero_zero_two_stage_screen10_r33_hidden_labels.json"
...
def load_json(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise R33Error(f"JSON root must be object: {path}")
    return value
...
def all_previous_exclusions(
    rows: list[dict[str, Any]], r30_config: dict[str, Any], r32_config: dict[str, Any]
) -> tuple[set[str], int, int, int]:
    excluded, r29_count, r30_count = r32.previous_research_exclusions(rows, r30_config)
    eligible = [row for row in rows if row["identity"] not in excluded]
    targets = [
        row for row in eligible
        if score(row) in {tuple(x) for x in r32_config["target_scores"]}
    ]
    controls = [
        row for row in eligible
        if score(row) in {tuple(x) for x in r32_config["control_scores"]}
    ]
    chosen = r30.choose_targets(
        targets,
        int(r32_config["pairs"]),
        int(r32_config["seed"]),
        int(r32_config["max_targets_per_competition"]),
    )
    matched = r30.match_controls(chosen, controls, "R30A")
    r32_ids: set[str] = set()
    for target, control, _ in matched:
        r32_ids.add(str(target["identity"]))
        r32_ids.add(str(control["identity"]))
    excluded |= r32_ids
    return excluded, r29_count, r30_count, len(r32_ids)


def build(
    config: dict[str, Any],
    r30_config: dict[str, Any],
    r32_config: dict[str, Any],
    status_path: Path,
    public_path: Path,
    ...
    if errors:
        raise R33Error(f"source extraction errors: {errors}")
    excluded, r29_count, r30_count, r32_count = all_previous_exclusions(
        rows, r30_config, r32_config
    )
    eligible = [row for row in rows if row["identity"] not in excluded]
    ...
    for index, (target, control, cost) in enumerate(matched, start=1):
        pair_id = f"R33-{index:02d}"
        pair_key = r30.sha256_text(
            f"{config['seed']}|{target['identity']}|{control['identity']}"
        )
        ...
        for side, row, role in ordering:
            ...
            hidden[f"side_{side}_final_score"] = (
                f"{row['home_goals_90']}-{row['away_goals_90']}"
            )
            hidden[f"side_{side}_identity"] = row["identity"]
        if r30.forbidden_candidate_key(candidates):
            raise R33Error(f"{pair_id}: public candidate leaked outcome key")
        public_pairs.append({
            "pair_id": pair_id,
    ...
    hidden_payload = {
        "schema_version": "v511_zero_zero_two_stage_screen10_r33_hidden_labels.1",
        "labels": hidden_rows,
    }
    hidden_seal = r30.sha256_text(r30.canonical(hidden_payload))
    public_payload = {
        "schema_version": "v511_zero_zero_two_stage_screen10_r33_public.1",
    ...
    public_path.write_text(
        json.dumps(public_payload, ensure_ascii=False, indent=2, allow_nan=False),
        encoding="utf-8",
    )
    r30.write_public_csv(public_csv_path, public_pairs)
    hidden_path.write_text(
        json.dumps(hidden_payload, ensure_ascii=False, indent=2, allow_nan=False),
        encoding="utf-8",
    )

    status = {
        "schema_version": "v511_zero_zero_two_stage_screen10_r33_status.1",
        "status": "PASS_R33_NEW_TWO_STAGE_ZERO_ZERO_BLIND_PACKET_BUILT",
        "classification": config["classification"],
        "formal_weight": 0,
        "pairs": len(public_pairs),
        "competitions": len({
            candidate["competition_id"]
            for pair in public_pairs for candidate in pair["candidates"]
        }),
        "r29_excluded_identity_count": r29_count,
        "r30_excluded_identity_count": r30_count,
        "r32_excluded_identity_count": r32_count,
        "total_excluded_identity_count": len(excluded),
        "public_packet_sha256": r30.sha256_text(
...
def main() -> None:
    ...
    parser.add_argument("--r32-config", type=Path, default=DEFAULT_R32_CONFIG)
    ...
    print(json.dumps(build(
        ...
        load_json(args.r30_config),
        load_json(args.r32_config),
        args.status,
        args.public,
        args.public_csv,
        args.hidden,
