...
ADAPTER_SCHEMA = "football3-frozen-source-compat-v3"
...
def _learn_team_map(history: list[rt.HistoryFixture], source_rows: list[dict[str, Any]]) -> tuple[dict[tuple[str, str], str], dict[str, Any]]:
    ...
    unmatched_schedule_rows: list[dict[str, str]] = []
    for s in source_rows:
        ...
        if not peers:
            unmatched_schedule_rows.append({"competition_id": comp, "source_date": date, "home_team": home, "away_team": away})
            continue
        appearances[(comp, home)] += 1; appearances[(comp, away)] += 1
        for f in peers:
            support[(comp, home)][f.home_team_name] += 1
            support[(comp, away)][f.away_team_name] += 1
    if len(unmatched_schedule_rows) > 5:
        raise rt.RuntimeGateError(f"too many source/V1 schedule date gaps: {len(unmatched_schedule_rows)}")
    ...
    for comp in sorted(source_teams_by_comp):
        for source_name in sorted(source_teams_by_comp[comp]):
            ...
            if best_n == second_n or ratio < 0.80:
                raise rt.RuntimeGateError(f"ambiguous source team mapping {key}: best={best_name}:{best_n}/{total} second={second_n}")
    ...
    return mapping, {
        ...
        "unmatched_schedule_row_n": len(unmatched_schedule_rows),
        "unmatched_schedule_rows": unmatched_schedule_rows,
        "mapping": evidence,
    }
...
def _load_xg_labels(history: list[rt.HistoryFixture], understat_db: Path, confirmation_dir: Path) -> tuple[dict[str, rt.XGLabel], dict[str, Any]]:
    formal: list[rt.HistoryFixture] = [r for r in history if r.competition_id in set(rt.BIG5.values()) and r.season in ("2022/23", "2023/24", "2024/25")]
    ...
    for f in formal:
        k = (f.competition_id, f.kickoff.date().isoformat(), rt._normalize_team(f.home_team_name), rt._normalize_team(f.away_team_name))
        if k in exact:
            raise rt.RuntimeGateError(f"formal XG join identity collision: {k}")
        exact[k] = f
        by_teams[(f.competition_id, rt._normalize_team(f.home_team_name), rt._normalize_team(f.away_team_name))].append(f)
    ...
    date_reconciliation: list[dict[str, Any]] = []
    for s in source_rows:
        comp = str(s["competition_id"])
        mh = team_map[(comp, str(s["home_team"]))]; ma = team_map[(comp, str(s["away_team"]))]
        sh = rt._normalize_team(mh); sa = rt._normalize_team(ma); source_date = str(s["source_date"])
        f = exact.get((comp, source_date, sh, sa))
        if f is None:
            sd = datetime.fromisoformat(source_date).date()
            candidates = [x for x in by_teams.get((comp, sh, sa), []) if abs((x.kickoff.date() - sd).days) <= 2]
            if len(candidates) != 1:
                raise rt.RuntimeGateError(f"date-reconciled XG identity ambiguous: {comp} {source_date} {mh} v {ma} candidates={len(candidates)}")
            f = candidates[0]
            date_reconciliation.append({
                "competition_id": comp, "fixture_id": f.fixture_id, "home_team": f.home_team_name, "away_team": f.away_team_name,
                "source_date": source_date, "v1_date": f.kickoff.date().isoformat(),
                "offset_days": (f.kickoff.date() - sd).days,
                "rule": "same_competition_and_mapped_teams_unique_within_plus_minus_2_calendar_days",
                "label_free": True,
            })
        ...
        if (hg, ag) != (f.home_goals, f.away_goals):
            conflicts.append({"fixture_id": f.fixture_id, "competition_id": f.competition_id, "season": f.season,
    ...
    if len(labels) != rt.EXPECTED_XG_JOIN_N:
        raise rt.RuntimeGateError(f"mapped XG identity join incomplete joined={len(labels)} expected={rt.EXPECTED_XG_JOIN_N}")
    conflicts.sort(key=lambda r: (r["date"], r["competition_id"], r["fixture_id"]))
    date_reconciliation.sort(key=lambda r: (r["source_date"], r["competition_id"], r["fixture_id"]))
    if len(date_reconciliation) > 5:
        raise rt.RuntimeGateError(f"too many date reconciliations: {len(date_reconciliation)}")
    return labels, {
        "adapter_schema": ADAPTER_SCHEMA, "joined_n": len(labels), "old_rows": rt.EXPECTED_XG_OLD_N,
        "confirmation_rows": 1752, **source_meta, "team_identity_map": map_meta,
        "date_reconciliation_n": len(date_reconciliation),
        "date_reconciliation_sha256": rt._sha_bytes(rt._canon_bytes(date_reconciliation)),
        "date_reconciliation": date_reconciliation,
        "cross_source_result_conflict_n": len(conflicts),
        "cross_source_result_conflicts_sha256": rt._sha_bytes(rt._canon_bytes(conflicts)),
...
def install() -> dict[str, Any]:
    ...
    meta["date_identity_reconciliation"] = "label-free exact mapped teams, unique candidate within +/-2 calendar days"
    return meta
