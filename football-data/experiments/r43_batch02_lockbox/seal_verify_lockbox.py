...
def make_lockbox():
    ...
    um=[str(e['match_id']) for e in u[:END]];ym=[str(e['match_id']) for e in y[:END]]
    if um!=ym:raise RuntimeError('U1/Y0 match sequence mismatch through event 41')
    batch_ids=set(um[START:END]);seen=settled_ids() & batch_ids
    if seen:raise RuntimeError(f'Batch02 already has settlement before seal:{sorted(seen)}')
    box={
      'schema_version':'football3-r43-batch02-pristine-lockbox-v1',
      ...
      'range':{'one_based_start_event':25,'one_based_end_event':41,'event_count':17},
      'r43u1':{
...
def audit_from_box(box):
    ...
    for key in ('r43u1','r43y0'):
        ...
        out[key]={'status':'PASS' if not errors else 'FAIL','current_event_count':len(events),'sealed_event_count':n,'sealed_prefix_head':head,'sealed_prefix_digest':pdig,'batch_segment_digest':sdig,'errors':errors}
    return out

def verify_and_write():
    box=load(LOCKBOX);ensure_batch01_prefix(load(U1).get('events') or [],load(Y0).get('events') or [])
    audits=audit_from_box(box);passed=all(v['status']=='PASS' for v in audits.values())
    summary={'schema_version':'football3-r43-batch02-lockbox-verification-v1','status':'PASS' if passed else 'FAIL','batch_id':box['batch_id'],'lockbox_status':box['status'],'governance':box['governance'],'range':box['range'],'r43u1_prefix_audit':audits['r43u1'],'r43y0_prefix_audit':audits['r43y0'],'action':'BATCH02_PREFIX_IMMUTABLE_CONTINUE_FORWARD_SETTLEMENT' if passed else 'STOP_BATCH02_SCORING_LOCKBOX_MISMATCH'}
