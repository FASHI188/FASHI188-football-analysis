...
def run():
 ...
 s={
  'schema_version':'football3-batch003-lock-v1','status':'LOCKED',
  'purpose':'fresh third independent 100-match pseudo-prospective batch for hierarchical draw candidate',
  ...
  'governance':{'safe_fields_accessed':list(SAFE),'source_files_contain_other_fields_but_not_accessed_for_selection':True,'outcome_fields_accessed':False,'selection_uses_results':False,'selection_uses_odds':False,'selection_uses_postmatch_stats':False,'cohort_locked_before_Batch003_candidate_predictions':True,'reveal_only_after_all_candidate_predictions_locked':True,'Batch002_diagnostics_not_used_for_Batch003_row_selection':True},
  'sources':src,'cohort_sha256':sha(lock),'rows':lock}
 OUT.mkdir(parents=True,exist_ok=True);(OUT/'batch003_locked_100.json').write_text(json.dumps(s,indent=2,ensure_ascii=False)+'\n',encoding='utf-8')
 print(json.dumps({'status':'LOCKED','selection':s['selection'],'cohort_sha256':s['cohort_sha256']},indent=2))

def verify():
 s=json.loads((OUT/'batch003_locked_100.json').read_text(encoding='utf-8'));g=s['governance']
 ...
 assert s['cohort_sha256']==sha(s['rows']);assert s['selection']['first_date']>='2026-04-12'
 print('BATCH003_LOCK_VERIFY_PASS')

if __name__=='__main__':
 if len(sys.argv)!=2 or sys.argv[1] not in {'run','verify'}:raise SystemExit('usage: lock_batch003.py {run|verify}')
