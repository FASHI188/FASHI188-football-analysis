...
def team_view(x,tid):
    h=str(x['home_team'])==str(tid)
    return {
      'date':x['date'],'gf':int(x['home_goals'] if h else x['away_goals']),'ga':int(x['away_goals'] if h else x['home_goals']),
      'xgf':float(x['home_xg'] if h else x['away_xg']),'xga':float(x['away_xg'] if h else x['home_xg']),
      'known':x['_known']
    }

def avg(v): return sum(v)/len(v) if v else 0.0

def form_features(pool,tid,cutoff):
    z=[team_view(x,tid) for x in pool if x['_known']<cutoff and (str(x['home_team'])==str(tid) or str(x['away_team'])==str(tid))]
    z.sort(key=lambda q:q['known']); out={}
    for n in (5,10):
      q=z[-n:]
...
def schedule_features(fixtures,tid,cutoff):
    q=fixtures[((fixtures.home_team_id.astype(str)==str(tid))|(fixtures.away_team_id.astype(str)==str(tid)))&(fixtures.date_utc<cutoff)].sort_values('date_utc')
    if len(q): rest=(cutoff-q.iloc[-1].date_utc).total_seconds()/86400.0
    else: rest=999.0
    o={'rest_days':float(rest)}
    for d in (7,14,30): o[f'matches_{d}d']=int((q.date_utc>=cutoff-pd.Timedelta(days=d)).sum())
    return o
...
def run():
    ...
    # Target fixture metadata only: intentionally no outcome/status columns.
    ...
    byid={str(int(x.id)):x for x in fx.itertuples(index=False)}; rows=[]
    for p in s['predictions']:
      ...
      hf=form_features(pool,hid,cutoff); af=form_features(pool,aid,cutoff); hs=schedule_features(fx,hid,cutoff); ass=schedule_features(fx,aid,cutoff)
      diff={}
