...
FEATURE='shot_lateral_width_fit_abs'
bp.FEATURE=FEATURE
common=bp.common
bmod=bp.bmod
EPS=1e-15
...
def main()->int:
    ...
    assert c['feature']['family']=='shot_lateral_width' and c['feature']['key']==FEATURE and c['feature']['source_receipt_is_authoritative'] is True
    ...
    frozen=common.build_frozen_baseline(a,'shot_lateral_width_candidate')
    ...
    if passed:
        ...
        final={'schema_version':'football3-shot-lateral-width-frozen-params-v1','status':'FROZEN_AFTER_DEVELOPMENT_PASS_BEFORE_ANY_CONFIRMATION','feature_family':'shot_lateral_width','feature_key':FEATURE,'standardization_mean':mu,'standardization_sd':sd,'beta_intercept':float(beta[0]),'beta_slope':float(beta[1]),'fixed_ridge':ridge,'training_seasons':[2020,2021,2022],'training_n':len(rows),'formal_weight':0,'historical_confirmation_2023_labels_opened':False,'prospective_1335_data_touched':False}
        common.write_json(a.out/'FROZEN_SHOT_LATERAL_WIDTH_PARAMS.json',final)
    out={'schema_version':'football3-prior-shot-lateral-width-candidate-result-v1','status':c['terminal']['pass'] if passed else c['terminal']['fail'],'scientific_pass':passed,'research_only':True,'formal_weight':0,'development_n':len(rows),'rolling_oos_n':len(test_rows),'source_max_season_loaded':source_audit['max_season'],'coverage':coverage,'source_receipt_audit':source_audit,'stage6_b_active_n':snaprec['active'],'historical_confirmation_2023_labels_opened':False,'prospective_1335_data_touched':False,'feature_family':'shot_lateral_width','feature_key':FEATURE,'fold_results':folds,'pooled':{'baseline':bm,'candidate':cm,'deltas':d,'exact_score_logloss':{'baseline':be,'candidate':ce},'total_goals_logloss':{'baseline':bt,'candidate':ct}},'max_outcome_probability_abs_delta':max_delta,'conditional_home_away_ratio_max_abs_error':max_ratio,'matrix_1x2_max_abs_error':matrix_err,'checks':checks,'frozen_params_written':final is not None,'next_step':'FIND_AND_PREREGISTER_GENUINELY_UNCONSUMED_HISTORICAL_CONFIRMATION_COHORT' if passed else 'CLOSE_SHOT_LATERAL_WIDTH_NO_RETUNE'}
    common.write_json(a.out/'shot_lateral_width_candidate_result.json',out); print(json.dumps(out,sort_keys=True)); return 0
