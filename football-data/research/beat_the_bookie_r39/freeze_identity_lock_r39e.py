...
def lane_ok(by,lane):
    ...
    if lane=='STRICT_COMMON5_ALL72':return len(set.intersection(*by))>=5
    if lane=='PER_HOUR5_ALL72':return all(c>=5 for c in counts)
    if lane=='PER_HOUR5_AT_LEAST60_PLUS_FINAL6':return sum(c>=5 for c in counts)>=60 and all(c>=5 for c in counts[-6:])
    if lane=='SIX_HOURLY_GRID5':return all(counts[i]>=5 for i in [0,6,12,18,24,30,36,42,48,54,60,66])
    raise RuntimeError(f'unknown lane {lane}')
def base3_ok(by):
    # R39C/R39D eligibility used stored suffixes corresponding to T-24h/T-6h/T-1h.
    idx=[71-24,71-6,71-1]
    common=set.intersection(*(by[i] for i in idx))
    return len(common)>=5
...
def load(source_dir,lane):
    ...
    for sanitized in ('odds_series_no_scores.csv.gz','odds_series_b_no_scores.csv.gz'):
        rawname=sanitized.replace('_no_scores','')
