...
def binary_ll(y: list[int], p: list[float]) -> float:
    ...
    for yy, pp in zip(y, p):
        ...
        s -= math.log(pp if yy else (1.0 - pp))
    return s / len(y)
...
def main() -> int:
    ...
    assert c["feature"]["family"] == "chance_burstiness"
    ...
    frozen = common.build_frozen_baseline(a, "chance_burstiness_candidate")
    ...
    fmap, event_audit = et.temporal_feature_map(a.db)
    ...
    if scientific_pass:
        ...
        final_params = {
            "schema_version": "football3-chance-burstiness-frozen-params-v1",
            "status": "FROZEN_AFTER_DEVELOPMENT_PASS_BEFORE_ANY_CONFIRMATION",
            "feature_family": "chance_burstiness",
            "feature_key": FEATURE,
        ...
        common.write_json(a.out / "FROZEN_CHANCE_BURSTINESS_PARAMS.json", final_params)

    out = {
        "schema_version": "football3-event-temporal-chance-burstiness-candidate-result-v1",
        ...
        "next_step": (
            ...
            if scientific_pass
            else "CLOSE_CHANCE_BURSTINESS_NO_RETUNE"
        ),
    }
    common.write_json(a.out / "chance_burstiness_candidate_result.json", out)
