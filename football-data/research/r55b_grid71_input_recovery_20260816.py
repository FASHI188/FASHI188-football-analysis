...
def fetch_blob(sha: str) -> str:
    ...
    req = urllib.request.Request(url, headers={'Accept':'application/vnd.github+json','User-Agent':'r55b-grid71-recovery/1.0'})
...
def grid71_prob(row):
    hs, ds, aas = [], [], []
    for b in range(1, 33):
        h = fodd(row.get(f'home_b{b}_71'))
        d = fodd(row.get(f'draw_b{b}_71'))
        a = fodd(row.get(f'away_b{b}_71'))
...
def main():
    ...
    with zipfile.ZipFile(args.archive) as zf:
        closing_name = member(zf, 'closing_odds.csv.gz')
        odds_name = member(zf, 'odds_series_b.csv.gz')

        with zf.open(closing_name) as raw, gzip.GzipFile(fileobj=raw) as gz, io.TextIOWrapper(gz, encoding='utf-8', newline='') as text:
            reader = csv.DictReader(text)
            required = {'match_id','league','match_date','home_team','away_team','home_score','away_score'}
            if not required.issubset(set(reader.fieldnames or [])):
                raise SystemExit(f'CLOSING_HEADER_FAIL: {sorted(required-set(reader.fieldnames or []))}')
            for row in reader:
                league = row['league']
                if league not in ELIGIBLE:
                    continue
                d = row['match_date'][:10]
                if not d or d >= CAL_CUTOFF:
                    continue
                key = (league, d, norm(row['home_team']), norm(row['away_team']))
                hit = ou.get(key)
                if not hit:
                    continue
                mid = row['match_id']
                if mid in calibration_candidates:
                    raise SystemExit(f'CAL_DUPLICATE_MATCH_ID: {mid}')
                try:
                    hs = int(float(row['home_score']))
                    aas = int(float(row['away_score']))
                except Exception:
                    continue
                calibration_candidates[mid] = {
                    'match_id':mid,
                    'league':league,
                    'match_date':d,
        ...
        wanted = set(calibration_candidates) | set(target_by_id)
        grid = {}
        with zf.open(odds_name) as raw, gzip.GzipFile(fileobj=raw) as gz, io.TextIOWrapper(gz, encoding='utf-8', newline='') as text:
            reader = csv.DictReader(text)
            required = {'match_id'} | {f'{o}_b{b}_71' for b in range(1,33) for o in ('home','draw','away')}
            if not required.issubset(set(reader.fieldnames or [])):
                raise SystemExit(f'ODDS_SERIES_HEADER_FAIL: missing={len(required-set(reader.fieldnames or []))}')
            for row in reader:
                mid = row['match_id']
                if mid not in wanted:
                    continue
                if mid in grid:
                    raise SystemExit(f'GRID71_DUPLICATE_MATCH_ID: {mid}')
                q = grid71_prob(row)
                if q is not None:
                    grid[mid] = q
    ...
    calibration.sort(key=lambda r: (r['match_date'], int(r['match_id'])))
    ...
    for r in calibration:
        cal_counts[r['league']] = cal_counts.get(r['league'], 0) + 1
    target_counts = {}
    for r in target:
        target_counts[r['league']] = target_counts.get(r['league'], 0) + 1
    ...
    status = 'PASS_COMPLETE_INPUT_RECOVERY' if len(calibration) >= 300 and len(target) == TARGET_N and target_recovered_sha == TARGET_SHA else 'FAIL_INPUT_RECOVERY_GATE'

    payload = {
        'schema':'R55B_GRID71_INPUT_RECOVERY_R1',
        ...
        'source_archive':{'release_tag':'football-data-archive-v1','asset_name':'archive.1.1.zip','sha_recheck_performed':False},
        'grid71_rule':{
            'bookmakers':32,
            'grid':71,
        ...
        'calibration_rule':{
            'cutoff_exclusive':'2016-03-01 00:00:00',
        ...
        'audit':{
            ...
            'calibration_by_league':cal_counts,
            'target_by_league':target_counts,
            'calibration_date_window':[calibration[0]['match_date'],calibration[-1]['match_date']] if calibration else None,
    ...
    print(json.dumps({'status':status, **payload['audit']}, ensure_ascii=False, indent=2, sort_keys=True))
