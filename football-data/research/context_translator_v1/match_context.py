...
class ScheduleTracker:
    ...
    def features(self,home:str,away:str,cutoff:str,round_index:int|None=None,season_matches:int=38)->ScheduleFeatures:
        if home==away:
            raise MatchContextError("home and away identity must differ")
        co=_dt(cutoff)
    ...
    def observe_fixture(self,home:str,away:str,cutoff:str)->None:
        co=_dt(cutoff); key=(home,away,co.isoformat())
        if key in self._observed:
            raise MatchContextError("duplicate schedule fixture observation")
        self._observed.add(key); self.times[home].append(co); self.times[away].append(co); self.times[home].sort(); self.times[away].sort()
        self.away_streak[home]=0; self.away_streak[away]+=1


def opponent_adjusted_mu(league_rate:float,team_for_rate:float,opponent_against_rate:float,*,exponent:float=0.85)->float:
    for x in (league_rate,team_for_rate,opponent_against_rate):
        if not math.isfinite(float(x)) or float(x)<=0:
            raise MatchContextError("opponent-strength audit rates must be positive finite")
    if not 0<exponent<=2:
        raise MatchContextError("invalid strength exponent")
    return league_rate*(team_for_rate/league_rate)**exponent*(opponent_against_rate/league_rate)**exponent


def venue_core_audit(base_mu_home:float,base_mu_away:float,*,neutral:bool=False)->dict[str,Any]:
    if base_mu_home<=0 or base_mu_away<=0 or not math.isfinite(base_mu_home+base_mu_away):
        raise MatchContextError("invalid V2 base intensities")
    return {"status":"V2_CORE_SINGLE_APPLICATION","neutral":bool(neutral),"translator_extra_venue_multiplier":1.0,
            "base_mu_home":float(base_mu_home),"base_mu_away":float(base_mu_away),
            "audit_sha256":_sha({"home":base_mu_home,"away":base_mu_away,"neutral":bool(neutral),"extra_multiplier":1.0})}
...
def fit_schedule_coefficients(rows:list[dict[str,Any]],*,iterations:int=400,learning_rate:float=0.003,ridge:float=30.0)->list[float]:
    beta=[0.0]*6; n=max(len(rows),1)
    for _ in range(iterations):
        grad=[0.0]*6
        for r in rows:
            x=list(map(float,r["x"])); bh=max(float(r["base_mu_home"]),1e-6); ba=max(float(r["base_mu_away"]),1e-6)
            y_h=float(r["home_goals"]); y_a=float(r["away_goals"]); tilt=max(-0.6,min(0.6,sum(b*v for b,v in zip(beta,x))))
            mh=bh*math.exp(tilt); ma=ba*math.exp(-tilt); scalar=(mh-y_h)-(ma-y_a)
            for j in range(6):grad[j]+=scalar*x[j]
        for j in range(6):
            grad[j]=grad[j]/n+ridge*beta[j]/n; beta[j]-=learning_rate*grad[j]; beta[j]=max(-0.20,min(0.20,beta[j]))
    return beta
...
def environment_adjustment(facts:list[dict[str,Any]]|None,cutoff:str)->dict[str,Any]:
    ...
    for f in facts:
        ...
        usable.append(f)
    return {"status":"CONTRACT_ONLY","log_mu_home_delta":0.0,"log_mu_away_delta":0.0,"uncertainty":0.25,"evidence_sha256":_sha(usable)}


def uncertainty_with_missing(base:float,missing_layers:int)->float:
    if base<0 or missing_layers<0:
        raise MatchContextError("invalid uncertainty inputs")
    return float(base)+0.12*int(missing_layers)
