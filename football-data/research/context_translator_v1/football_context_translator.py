...
class LayerAdjustment:
    ...
    def schema_dict(self)->dict[str,Any]:
        ...
        for v in (self.log_mu_home_delta,self.log_mu_away_delta,self.uncertainty):
            if not math.isfinite(float(v)):raise ValueError("nonfinite layer adjustment")
...
def team_state(team_id:str,attack:float,defence:float,sample_size:float,uncertainty:float)->dict[str,Any]:
    ...
    payload["state_sha256"]=canonical_sha(payload);return payload
...
def build_plan(*,match_id:str,cutoff:str,base_mu_home:float,base_mu_away:float,home_team_state:dict[str,Any],away_team_state:dict[str,Any],
               ...
               player_status:str="BLOCKED_DATA",coverage_grade:str="TEAM_ONLY",bench_substitution:LayerAdjustment|None=None)->dict[str,Any]:
    if base_mu_home<=0 or base_mu_away<=0:raise ValueError("base mus must be positive")
    if not scenarios or abs(sum(float(x.probability) for x in scenarios)-1.0)>1e-9:raise ValueError("scenario probabilities must sum to one")
    bench=bench_substitution or LayerAdjustment("CONTRACT_ONLY",0.0,0.0,0.0,None)
    psha=None if not player_vectors else canonical_sha({k:v.to_dict() for k,v in sorted(player_vectors.items())})
    internal=[]
    for sc in scenarios:
        h_att=h_def=h_gk=a_att=a_def=a_gk=0.0;lineup_unc=float(sc.uncertainty)
        if player_vectors and sc.route!="LINEUP_UNKNOWN":
            ...
            lineup_unc=max(lineup_unc,(h_unc+a_unc)/2.0)
        player_home=h_att-a_def-a_gk;player_away=a_att-h_def-h_gk
        dh=player_home+bench.log_mu_home_delta+coach_tactical.log_mu_home_delta+match_context.log_mu_home_delta+process_hazard.log_mu_home_delta
        da=player_away+bench.log_mu_away_delta+coach_tactical.log_mu_away_delta+match_context.log_mu_away_delta+process_hazard.log_mu_away_delta
        dh=max(-0.70,min(0.70,dh));da=max(-0.70,min(0.70,da));lineup_unc+=max(0.0,bench.uncertainty)
        internal.append({"scenario":sc,"base_mu_home":base_mu_home,"base_mu_away":base_mu_away,
                         ...
                         "bench_home":bench.log_mu_home_delta,"bench_away":bench.log_mu_away_delta,
    ...
    return {"match_id":match_id,"cutoff":cutoff,"base_mu_home":base_mu_home,"base_mu_away":base_mu_away,
            ...
            "coach_tactical":coach_tactical,"match_context":match_context,"process_hazard":process_hazard,"bench_substitution":bench,
