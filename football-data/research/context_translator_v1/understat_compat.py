...
def _columns(con: sqlite3.Connection, table: str) -> dict[str, str]:
    return {str(r[1]).lower(): str(r[1]) for r in con.execute(f"pragma table_info({_q(table)})")}


def _find_table(con: sqlite3.Connection, required: set[str]) -> tuple[str, dict[str, str]]:
    found=[]
    ...
    if len(found)!=1:
        raise UnderstatCompatError(f"dataset table discovery expected exactly one {sorted(required)}; found={[x[0] for x in found]}")
    return found[0]


def _download_dataset(out: pathlib.Path) -> tuple[pathlib.Path, dict[str, Any]]:
    global _DB_PATH,_ARCHIVE_SHA,_ARCHIVE_BYTES,_GAME_TABLE,_LINEUP_TABLE
    if _DB_PATH is not None:
        return _DB_PATH, json.load(open(out/'understat_dataset_receipt.json'))
    ...
    try:
        ...
        raise UnderstatCompatError(f"public Kaggle Understat dataset download failed without credentials: {type(exc).__name__}: {exc}") from exc
    if len(raw)<1000:
        raise UnderstatCompatError(f"public dataset response too small: {len(raw)} bytes")
    _ARCHIVE_SHA=hashlib.sha256(raw).hexdigest();_ARCHIVE_BYTES=len(raw)
    try:
        z=zipfile.ZipFile(io.BytesIO(raw))
    except Exception as exc:
        raise UnderstatCompatError("public dataset response is not a ZIP archive") from exc
    bad=z.testzip()
    ...
    if not dbs:
        raise UnderstatCompatError(f"public dataset contains no SQLite database; members={members[:40]}")
    # Prefer the documented understat database; fail closed on multiple equally plausible DBs.
    ranked=sorted(dbs,key=lambda x:(0 if pathlib.Path(x).name.lower()=='understat.db' else 1,len(x),x))
    if len(ranked)>1 and pathlib.Path(ranked[0]).name.lower()!='understat.db':
        raise UnderstatCompatError(f"ambiguous SQLite database members: {ranked}")
    ...
    try:
        _GAME_TABLE,gcols=_find_table(con,{'id','date','team_h','team_a'})
        _LINEUP_TABLE,lcols=_find_table(con,{'match_id','player_id','team_id','position','player','time','xg','xa','xgchain','xgbuildup'})
    finally: con.close()
    _DB_PATH=tmp
    receipt={
        'schema_version':'football3-understat-public-dataset-v1',
        'provider':'Cody Tipton player stats per game - Understat',
        'source_page':KAGGLE_PAGE,'download_url':KAGGLE_DOWNLOAD,
        'license':'MIT (dataset page declaration)',
        'archive_sha256':_ARCHIVE_SHA,'archive_bytes':_ARCHIVE_BYTES,'zip_crc':'PASS',
        'database_member':ranked[0],'database_sha256':hashlib.sha256(tmp.read_bytes()).hexdigest(),'database_bytes':tmp.stat().st_size,
        'game_table':_GAME_TABLE,'lineup_table':_LINEUP_TABLE,
        'query_boundary':{
            'identity':'SELECT only id,date,team_h,team_a,h_id,a_id,league,season where present; no score/result/xG columns',
            'prior_player_state':'SELECT only match_id,player_id,team_id,position,player,time,h_a,xG,xA,xGChain,xGBuildup after that match prediction batch is frozen and release_at is reached',
            'target_match_player_rows_before_own_prediction':0,
            'season_aggregate_player_rows':0,
        },
        'collected_at':core.iso(datetime.now(timezone.utc)),
    }
    core.dump(out/'understat_dataset_receipt.json',receipt)
    return tmp,receipt


def understat_identity(v2_rows, out):
    global _GAME_SIDE
    db,receipt=_download_dataset(out)
    con=sqlite3.connect(db)
    try:
        cols=_columns(con,_GAME_TABLE or '')
        safe=['id','date','team_h','team_a']+[x for x in ('h_id','a_id','league','season') if x in cols]
        sql='select '+','.join(_q(cols[x]) for x in safe)+' from '+_q(_GAME_TABLE or '')
        rows=[]
        for tup in con.execute(sql): rows.append(dict(zip(safe,tup)))
    finally: con.close()
    ...
    for x in rows:
        ...
        rec={'understat_match_id':str(x['id']),'date':date,'home':home,'away':away,'source_home_id':None if x.get('h_id') is None else str(x.get('h_id')),'source_away_id':None if x.get('a_id') is None else str(x.get('a_id'))}
        idx[(date,core.norm(home),core.norm(away))].append(rec)
    mapping={};audit=[]
    for r in v2_rows:
        ...
        z=cand[0];fid=str(r['fixture_id']);mapping[fid]=z['understat_match_id'];_GAME_SIDE[z['understat_match_id']]=(z.get('source_home_id') or '',z.get('source_away_id') or '')
        audit.append({'fixture_id':fid,**z})
    if len(mapping)!=core.FULL_SEASON_N or len(set(mapping.values()))!=core.FULL_SEASON_N:
        raise UnderstatCompatError(f"public dataset full-season identity is not one-to-one {core.FULL_SEASON_N}: {len(mapping)}")
    rec={'schema_version':'football3-understat-epl-2023-24-identity-v3','transport':'PUBLIC_KAGGLE_DATASET_SQLITE_SAFE_COLUMN_PROJECTION','source_url':KAGGLE_PAGE,'archive_sha256':receipt['archive_sha256'],'database_sha256':receipt['database_sha256'],'collected_at':core.iso(datetime.now(timezone.utc)),'identity_rule':'date+canonical_home+canonical_away; SQL projection excludes score/result/xG columns','mapped_n':len(mapping),'result_fields_read':False,'xg_fields_read_for_identity':False,'rows_sha256':core.canon(audit)}
...
def understat_roster(match_id: str, home_tid: str, away_tid: str, release_at: str) -> dict[str, Any]:
    if _DB_PATH is None or _LINEUP_TABLE is None or _ARCHIVE_SHA is None:
        raise UnderstatCompatError('dataset not initialized before prior-match release')
    con=sqlite3.connect(_DB_PATH)
    try:
        cols=_columns(con,_LINEUP_TABLE)
        needed=['match_id','player_id','team_id','position','player','time','xg','xa','xgchain','xgbuildup']
        optional=[x for x in ('h_a','positionorder') if x in cols]
        sql='select '+','.join(_q(cols[x]) for x in needed+optional)+' from '+_q(_LINEUP_TABLE)+' where '+_q(cols['match_id'])+'=?'
        rawrows=[dict(zip(needed+optional,t)) for t in con.execute(sql,(match_id,))]
    finally: con.close()
    if not rawrows: raise UnderstatCompatError(f'prior released match has no lineup rows: {match_id}')
    source_h,source_a=_GAME_SIDE.get(str(match_id),('',''))
    out_usage=defaultdict(list);events=[];aliases=defaultdict(list);safe_rows=[]
    for x in rawrows:
        ...
        rec={'player_id':pid,'started':started,'appeared':minutes>0,'minutes':minutes,'role':role,'known_at':release_at,'player_name':name};out_usage[tid].append(rec);aliases[tid].append({'player_id':pid,'player_name':name})
    ...
    for tid,rows in out_usage.items():
        if len([x for x in rows if x['started']])!=11:
            # Dataset position semantics may be incomplete; do not fabricate a reference XI.
            for x in rows: x['started']=False
    subset=core.canon(sorted(safe_rows,key=lambda x:(x['h_a'],x['player_id'])))
    return {'source_url':KAGGLE_PAGE+'#lineup_stats','source_sha256':subset,'source_archive_sha256':_ARCHIVE_SHA,'source_bytes':len(json.dumps(safe_rows,sort_keys=True,separators=(',',':')).encode()),'release_at':release_at,'usage':dict(out_usage),'events':events,'aliases':dict(aliases),'prohibited_target_fields_retained':False,'rating_field_used':False,'transport':'PUBLIC_KAGGLE_DATASET_SQLITE_SAFE_COLUMN_PROJECTION'}
...
def install()->None:
    core.understat_identity=understat_identity;core.understat_roster=understat_roster;core.resolve_source_name=strict_resolve_source_name
...
def main()->int:
    ...
    if a.cmd=='predict': result=core.run_prediction(pathlib.Path(a.v2),pathlib.Path(a.out))
    else: result=core.score(pathlib.Path(a.v2),pathlib.Path(a.label_vault),pathlib.Path(a.out))
