...
def _usage_as_of(snapshot: dict[str, Any]) -> tuple[dict[str, list[dict[str, Any]]], int]:
    ...
    for tid, rows in (snapshot.get("usage") or {}).items():
        ...
        for row in rows:
            ...
            if known < cutoff:
                ...
                removed += 1
        usage[str(tid)] = kept
    expected = core.canon({"vectors": snapshot.get("vectors") or {}, "usage": usage})
    if expected != str(snapshot["state_sha256"]):
        raise ShardError(f"checkpoint state SHA cannot be recovered at fixture {snapshot.get('fixture_id')}")
...
def repair_source(source: pathlib.Path, out: pathlib.Path) -> dict[str, Any]:
    ...
    for snapshot in states:
        usage, removed = _usage_as_of(snapshot)
        row = dict(snapshot)
        row["usage"] = usage
        repaired.append(row)
    ...
    payload_names = list(manifest["payload"].keys())
    ...
    new_manifest["checkpoint_resume"] = {
        ...
        "repair_rule": "drop usage rows whose known_at is not strictly before that fixture T-15 cutoff; require reconstructed vectors+usage SHA to equal the originally frozen per-fixture state_sha256",
