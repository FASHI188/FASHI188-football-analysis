...
def main():
    ...
    raw=collections.defaultdict(lambda:{'h':[],'a':[]}); n=0; y_nonblank=0
    for r in con.execute(f"select e.match_id,e.h_a,e.situation,e.shotType,e.xG,e.X,e.Y,e.minute from game_events e join general_game_stats g on g.id=e.match_id where g.league in ({qs}) and g.season between 2014 and 2022 and e.situation!='Penalty' order by e.match_id,e.minute,e.id",LEAGUES):
        ...
        if finite(d.get('Y')) is not None: y_nonblank+=1
    con.close()
    ...
    for day in sorted(byday):
        ...
        for g in games:
            ...
            r={'fixture_id':f"understat:{g['fid']}",'season':g['season'],'league':g['league'],'day':day,'shot_lateral_width_fit_abs':None,'central_xg_fit_abs':None,'shot_xg_hhi_fit_abs':None,'setpiece_xg_share_fit_abs':None,'temporal_xg_hhi_fit_abs':None,'shot_bodypart_hhi_fit_abs':None,'target_current_event_access':False,'target_result_access':False,'target_score_access':False,'target_match_xg_access':False}
            if hp is not None and ap is not None:
                r['shot_lateral_width_fit_abs']=matchup(hp,ap,'width'); r['central_xg_fit_abs']=matchup(hp,ap,'central'); r['shot_xg_hhi_fit_abs']=matchup(hp,ap,'shotq'); r['setpiece_xg_share_fit_abs']=matchup(hp,ap,'setp'); r['temporal_xg_hhi_fit_abs']=matchup(hp,ap,'time'); r['shot_bodypart_hhi_fit_abs']=matchup(hp,ap,'body')
            recs.append(r)
        for g in games:
    ...
    covered=[r for r in recs if r['shot_lateral_width_fit_abs'] is not None]; vals=[float(r['shot_lateral_width_fit_abs']) for r in covered]
    compkeys=('central_xg_fit_abs','shot_xg_hhi_fit_abs','setpiece_xg_share_fit_abs','temporal_xg_hhi_fit_abs','shot_bodypart_hhi_fit_abs')
    ...
    coverage=len(covered)/len(recs) if recs else 0.0; yfrac=y_nonblank/n if n else 0.0; g=c['gates']
    checks={'target_n_exact':len(recs)==int(g['target_n_exact']),'per_season_n_exact':all(counts[str(s)]==int(g['per_season_n_exact']) for s in TARGET_SEASONS),'minimum_match_feature_coverage':coverage>=float(g['minimum_match_feature_coverage']),'minimum_y_nonblank_fraction':yfrac>=float(g['minimum_y_nonblank_fraction']),'maximum_absolute_redundancy_correlation':maxcorr<=float(g['maximum_absolute_redundancy_correlation'])}
    passed=all(checks.values()); out={'schema_version':'football3-prior-shot-lateral-width-source-audit-result-v1','status':c['terminal']['pass'] if passed else c['terminal']['fail'],'source_only':True,'model_fit':0,'candidate_probability':0,'outcome_label_access':0,'target_n':len(recs),'target_counts':counts,'match_feature_covered_n':len(covered),'match_feature_coverage':coverage,'nonpenalty_event_n':n,'y_nonblank_n':y_nonblank,'y_nonblank_fraction':yfrac,'valid_history_rowsides':valid,'skipped_history_rowsides':skipped,'feature_key':c['feature']['key'],'redundancy_correlations':cors,'maximum_absolute_redundancy_correlation':maxcorr,'checks':checks,'historical_confirmation_2023_labels_opened':False,'prospective_1335_data_touched':False,'formal_weight':0,'next_step':'FREEZE_ONE_SHOT_LATERAL_WIDTH_CANDIDATE_NO_SEARCH' if passed else 'CLOSE_SHOT_LATERAL_WIDTH_AS_DUPLICATE_OR_INSUFFICIENT_SOURCE'}
    a.out.mkdir(parents=True,exist_ok=True); (a.out/'shot_lateral_width_source_audit.json').write_text(json.dumps(out,sort_keys=True,indent=2)+'\n')
    with (a.out/'shot_lateral_width_prematch_receipts.jsonl').open('w',encoding='utf-8') as f:
