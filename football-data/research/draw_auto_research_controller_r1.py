...
def initial_checkpoint(spec: dict[str, Any], authorization: dict[str, Any], identity: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "spec_digest": canonical_json_sha256(spec), "identity_digest": canonical_json_sha256(identity),
        "next_candidate_index": 0, "completed_candidates": [], "failed_candidates": [],
        "duplicate_prediction_candidates": [], "prediction_fingerprints": [],
        "batch_index": 0, "consecutive_stagnant_batches": 0,
        "best_ranking_score": None, "top5": [], "eligible_challenger": None,
        "cumulative_runtime_seconds": 0.0, "stop_reason": None, "safety_failure": None,
        "formal_weight": 0, "repository_writeback": 0, "provider_requests": 0,
        "api_football_requests": 0, "new_data_collection": False,
        "active_batch_candidate_ids": [], "active_batch_records": [],
        "active_batch_start_best_score": None, "active_batch_started_at": None,
    }
...
def load_checkpoint(state_dir: pathlib.Path, spec: dict[str, Any], authorization: dict[str, Any], identity: dict[str, Any]) -> dict[str, Any]:
    ...
    expected = {
        ...
        "user_authorization_record": "rec0WJJzXiuDvAqSb", "formal_weight": 0, "repository_writeback": 0,
    }
...
def candidate_summary(result: dict[str, Any], result_path: pathlib.Path) -> dict[str, Any]:
    return {
        ...
        "profile": result["candidate"]["profile"], "basis_variant": result["candidate"]["basis_variant"],
        "status": result["status"], "ranking_score": result["ranking_score"],
        "pooled_candidate_metrics": result["pooled_candidate_metrics"],
        "pooled_baseline_metrics": result["pooled_baseline_metrics"], "pooled_delta": result["pooled_delta"],
        "challenger_gate": result["challenger_gate"],
        "prediction_fingerprint": result["candidate_prediction_fingerprint"],
        "runtime_seconds": result["runtime_seconds"], "result_file": result_path.name,
        "result_sha256": sha256_file(result_path), "recorded_at": utc_now(),
        "data_status": "VIEWED_DEVELOPMENT_DATA",
    }
...
def batch_markdown(checkpoint: dict[str, Any], batch_records: list[dict[str, Any]], spec: dict[str, Any]) -> str:
    lines = [
        ...
        f"- 本批记录：{len(batch_records)}", f"- 累计完成：{len(checkpoint['completed_candidates'])}",
...
def final_markdown(checkpoint: dict[str, Any], spec: dict[str, Any]) -> str:
    lines = [
        ...
        "## 停止状态", "", f"- 状态：`{checkpoint['status']}`", f"- 停止原因：`{checkpoint['stop_reason']}`",
        f"- 完成候选：{len(checkpoint['completed_candidates'])}", f"- 失败候选：{len(checkpoint['failed_candidates'])}",
        f"- 等价预测淘汰：{len(checkpoint['duplicate_prediction_candidates'])}", f"- 累计运行：{checkpoint['cumulative_runtime_seconds']:.1f} 秒", "",
        "## Top 5", "",
...
def build_manifest(state_dir: pathlib.Path, checkpoint: dict[str, Any], spec: dict[str, Any], identity: dict[str, Any]) -> None:
    ...
    atomic_json(state_dir / "manifest.json", {
        "schema_version": "DRAW-AUTO-RESEARCH-ARTIFACT-MANIFEST-R1.4", "generated_at": utc_now(),
        "data_status": "VIEWED_DEVELOPMENT_DATA", "checkpoint_status": checkpoint.get("status"),
        "stop_reason": checkpoint.get("stop_reason"), "authorization_digest": checkpoint.get("authorization_digest"),
        "frozen_code_head": checkpoint.get("frozen_code_head"), "spec_digest": canonical_json_sha256(spec),
        "identity_digest": canonical_json_sha256(identity), "files": files,
        "repository_writeback": 0, "formal_weight": 0, "provider_requests": 0, "api_football_requests": 0,
    })
...
def run_batch(state_dir: pathlib.Path, run_id: str, run_attempt: str, max_run_seconds: int) -> int:
    ...
    spec = read_json(SPEC_PATH); identity = read_json(IDENTITY_PATH); authorization = validate_authorization(spec, identity)
    ...
    if checkpoint["status"] != "RUNNING":
        ...
        return 0
    checkpoint["last_run_id"] = str(run_id); checkpoint["last_run_attempt"] = str(run_attempt)
    catalog = candidate_catalog(); by_id = {item["candidate_id"]: item for item in catalog}
    ...
    if not active:
        ...
        if not remaining:
            checkpoint["status"] = "STOPPED"; checkpoint["stop_reason"] = "MAXIMUM_200_CANDIDATES_REACHED"
            _save_checkpoint(state_dir, checkpoint, base_runtime, started)
            (state_dir / "final_report.md").write_text(final_markdown(checkpoint, spec), encoding="utf-8")
            build_manifest(state_dir, checkpoint, spec, identity); return 0
        active = remaining[: int(spec["budget"]["batch_size"])]
        checkpoint["active_batch_candidate_ids"] = active; checkpoint["active_batch_records"] = []
        checkpoint["active_batch_start_best_score"] = checkpoint["best_ranking_score"]; checkpoint["active_batch_started_at"] = utc_now()
        _save_checkpoint(state_dir, checkpoint, base_runtime, started)
    rows = load_rows(spec); folds = build_outer_folds(rows)
    ...
    for candidate_id in active:
        ...
        if time.monotonic() - started >= max_run_seconds or base_runtime + (time.monotonic() - started) >= float(spec["budget"]["maximum_cumulative_seconds"]):
            break
        candidate = by_id[candidate_id]; result_path = state_dir / "results" / f"{candidate_id}.json"
        try:
            ...
            if not result["prediction_fingerprint_unique"]:
                ...
                append_jsonl(state_dir / "ledger.jsonl", record); checkpoint["duplicate_prediction_candidates"].append(candidate_id)
            else:
                validate_candidate_result(result); atomic_json(result_path, result)
                record = {"record_type": "CANDIDATE", **candidate_summary(result, result_path)}
                append_jsonl(state_dir / "ledger.jsonl", record); checkpoint["completed_candidates"].append(candidate_id)
                seen.add(result["candidate_prediction_fingerprint"]); checkpoint["prediction_fingerprints"] = sorted(seen)
            ...
            append_jsonl(state_dir / "ledger.jsonl", record); checkpoint["failed_candidates"].append({"candidate_id": candidate_id, "error": str(exc)})
        checkpoint["active_batch_records"].append(record); checkpoint["next_candidate_index"] += 1
        update_rankings(state_dir, checkpoint); _save_checkpoint(state_dir, checkpoint, base_runtime, started); attempted_ids.add(candidate_id)
    active_complete = all(candidate_id in attempted_ids for candidate_id in active)
    if active_complete:
        checkpoint["batch_index"] += 1
        before_best = checkpoint.get("active_batch_start_best_score"); current_best = checkpoint["best_ranking_score"]
        ...
        summary_path = state_dir / "summaries" / f"batch_{checkpoint['batch_index']:02d}.md"; summary_path.parent.mkdir(parents=True, exist_ok=True)
        summary_path.write_text(batch_markdown(checkpoint, list(checkpoint["active_batch_records"]), spec), encoding="utf-8")
        checkpoint["active_batch_candidate_ids"] = []; checkpoint["active_batch_records"] = []
        checkpoint["active_batch_start_best_score"] = None; checkpoint["active_batch_started_at"] = None
    reason = stop_reason(checkpoint, spec)
    if reason:
        checkpoint["status"] = "FAILED_SAFETY" if reason == "SAFETY_GATE_FAILURE" else "STOPPED"; checkpoint["stop_reason"] = reason
...
def probe(state_dir: pathlib.Path) -> int:
    if not (state_dir / "checkpoint.json").exists():
        if (state_dir / "run_failure_receipt.json").exists():
            print(json.dumps({"status": "FAILED_WITHOUT_CHECKPOINT", "should_continue": False})); return 0
        print(json.dumps({"status": "NEW", "should_continue": True})); return 0
    checkpoint = read_json(state_dir / "checkpoint.json")
    print(json.dumps({"status": checkpoint.get("status"), "should_continue": checkpoint.get("status") == "RUNNING"})); return 0


def main() -> int:
    parser = argparse.ArgumentParser(); parser.add_argument("--state-dir", type=pathlib.Path, required=True)
    parser.add_argument("--mode", choices=("run-batch", "probe"), default="run-batch")
    parser.add_argument("--run-id", default=os.environ.get("GITHUB_RUN_ID", "local")); parser.add_argument("--run-attempt", default=os.environ.get("GITHUB_RUN_ATTEMPT", "1"))
    parser.add_argument("--max-run-seconds", type=int, default=19800); args = parser.parse_args()
    if args.mode == "probe": return probe(args.state_dir)
