...
def choose_safe_seed_cutoff(sample: list[rt.HistoryFixture]) -> datetime:
    ...
    # The seed is defined relative to the first prediction cutoff, not to a release
    # event. Exact-cutoff labels remain eligible in reference_advance; fixture
    # freezes at that same timestamp remain deferred.
...
def reference_events(history: list[rt.HistoryFixture], labels: dict[str, rt.XGLabel]) -> list[dict[str, Any]]:
    ...
    This deliberately does not call runtime.history_delta_events. It reconstructs
    the event stream from frozen history, frozen xG labels and the already-installed
    formal adjudication manifest so reference and FAST paths cannot pass by sharing
    ...
    for row in history:
        ...
        if x is not None:
            events.append({
                "event_type": "FIXTURE_FREEZE", "event_at": row.kickoff,
                "row": row, "x": x,
            })
        ...
        if entry is None:
            ...
            events.append({
                "event_type": "LABEL_RELEASE", "event_at": release,
                "row": row, "x": x, "enters_v1": True, "enters_xg": x is not None,
        ...
        events.append({
            "event_type": "LABEL_RELEASE", "event_at": x.label.release_at,
            "row": row, "x": x, "enters_v1": False, "enters_xg": True,
        ...
        events.append({
            "event_type": "LABEL_RELEASE", "event_at": formal_at,
            "row": row, "x": None, "enters_v1": True, "enters_xg": False,
            "home_goals": row.home_goals, "away_goals": row.away_goals,
        })
...
def _reference_rebuild_v1(state: rt.hxg.ChallengerState) -> None:
    """Rebuild from every V1 label available at the current PIT boundary."""
...
def reference_apply_group(state: rt.hxg.ChallengerState, group: list[dict[str, Any]]) -> None:
    """Apply one event_at transaction without violating kickoff-ordered V1 state."""
    if not group:
        return
    ...
    for _, items in sorted(by_kickoff.items()):
        ...
        if xitems:
            ...
            labels = {event["row"].fixture_id: event["x"].label for event in xitems}
            state.apply_released_batch(fixtures, labels, as_of=event_at, update_base=False)
...
def reference_advance(state: rt.hxg.ChallengerState, events: list[dict[str, Any]], pos: int, cutoff: datetime) -> int:
    """Advance exactly once per event_at group and preserve deferred cutoff freezes."""
    while pos < len(events):
        at = events[pos]["event_at"]
        same: list[dict[str, Any]] = []
        j = pos
        while j < len(events) and events[j]["event_at"] == at:
            same.append(events[j])
            j += 1
        if at > cutoff:
            break
        if at < cutoff:
            reference_apply_group(state, same)
            pos = j
            continue

        # At the exact cutoff, released labels are PIT-eligible but a fixture freeze
        # at that same instant is not. LABEL_RELEASE sorts before FIXTURE_FREEZE, so
        # consume the complete eligible prefix exactly once and leave freezes queued.
        releases = [event for event in same if event["event_type"] == "LABEL_RELEASE"]
        if releases:
            reference_apply_group(state, releases)
            pos += len(releases)
        if pos < j and any(events[k]["event_type"] == "LABEL_RELEASE" for k in range(pos, j)):
            raise AssertionError("reference event ordering violated at cutoff boundary")
        if pos == j:
            pos = j
        break
    return pos
...
def benchmark_paths(history, labels, source, identity, sample, tmp: Path) -> dict[str, Any]:
    """Benchmark a genuinely FAST-eligible window; late V1 settlement remains FULL-only."""
    mid = len(sample) // 2
...
def _sync_core_overrides() -> None:
    # Keep legacy callers that monkeypatch the public test_runtime module working.
    core.choose_safe_seed_cutoff = globals()["choose_safe_seed_cutoff"]
    core.reference_events = globals()["reference_events"]
    core.reference_apply_group = globals()["reference_apply_group"]
    core.reference_advance = globals()["reference_advance"]
    core.benchmark_paths = globals()["benchmark_paths"]
    core.run_equivalence = globals()["run_equivalence"]
