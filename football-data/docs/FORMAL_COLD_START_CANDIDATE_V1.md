...
`GITHUB_BRANCH_DEFAULT_ROUTER_ACTIVE_FORMAL_WEIGHT_0`

This is the GitHub-branch default coverage router authorized on 2026-08-22. GitHub
is its only formal persistence and governance source. It is not a published
production artifact or scientific promotion. The
...
- `local_default_activation = true` (legacy CLI compatibility field; it grants no local governance authority);
...
## GitHub-only governance

The unique formal CURRENT is `football-data/governance/CURRENT_唯一正式规则.md`, and the active fact gate is `football-data/governance/FOOTBALL3_GPT_FACT_GATE_V1.md`. Local files, chat copies and Airtable records cannot activate or override this candidate. A web GPT may claim football3 output only after a real execution interface invokes the exact GitHub version.
