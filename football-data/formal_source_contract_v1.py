...
SCHEMA = "football3-formal-source-contract-adapter-v2"
_ORIGINAL_BUILD = rt.build_production_state_at_cutoff
_ORIGINAL_RESOLVE = rt.resolve_state_for_cutoff
_AUDIT: dict[str, Any] = {}


def _season_token(year: int) -> str:
...
def _source_rows(understat_db: Path, confirmation_dir: Path) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    ...
    for r in raw:
        ...
        rows.append({
            "competition_id": rt.BIG5[str(r["league"])], "season": _season_token(int(r["season"])),
...
def _learn_team_map(history: list[rt.HistoryFixture], source_rows: list[dict[str, Any]]) -> tuple[dict[tuple[str, str], str], dict[str, Any]]:
    ...
    return mapping, {"method": "competition+calendar_date+home_away_role_schedule_cooccurrence_only",
...
def _audit_and_labels(history: list[rt.HistoryFixture], understat_db: Path, confirmation_dir: Path):
    ...
    by_schedule: dict[tuple[str, str, str, str], list[rt.HistoryFixture]] = defaultdict(list)
    for f in formal:
        ...
        exact[k] = f
        by_schedule[(f.competition_id, f.season, h, a)].append(f)
    ...
    time_gaps: list[dict[str, Any]] = []; result_conflicts: list[dict[str, Any]] = []; boundaries: list[datetime] = []
    for s in source_rows:
        comp = str(s["competition_id"]); season = str(s["season"])
        ...
        if f is None:
            candidates = sorted(by_schedule.get((comp, season, h, a), []), key=lambda x: (x.kickoff, x.fixture_id))
            boundaries.append(candidates[0].kickoff if len(candidates) == 1 else s["source_kickoff"])
            time_gaps.append({
                "competition_id": comp, "season": season, "source_date": source_date,
                "source_kickoff": s["source_kickoff"].isoformat(), "source_release_at": s["release_at"].isoformat(),
                "home_team": mh, "away_team": ma, "schedule_identity_candidate_n": len(candidates),
                "candidate_v1_dates": [x.kickoff.date().isoformat() for x in candidates],
                "candidate_fixture_ids": [x.fixture_id for x in candidates],
                "reason": "source/formal time semantics differ; quarantined rather than shifting kickoff or release_at",
            })
            continue
        ...
        if (hg, ag) != (f.home_goals, f.away_goals):
            ...
            result_conflicts.append({
                ...
                "xg_release_at": s["release_at"].isoformat(),
                "reason": "official/awarded versus on-field result semantics are not defined by the formal wiring contract; no substitution",
            })
            continue
    ...
    boundary = min(boundaries) if boundaries else datetime.max.replace(tzinfo=timezone.utc)
    audit = {
        "schema_version": SCHEMA, "status": "SAFE_PREFIX_PLUS_EXPLICIT_QUARANTINE",
        "expected_join_n": rt.EXPECTED_XG_JOIN_N, "safe_join_n": len(labels),
        "time_semantic_gap_n": len(time_gaps), "result_conflict_n": len(result_conflicts),
        "time_semantic_gaps": time_gaps, "result_conflicts": result_conflicts,
        "team_identity_map": map_meta, "verified_complete_before_exclusive": boundary.isoformat(),
        "policy": {
            "team_aliases": "identity-only schedule cooccurrence; no result/xG labels used",
            "time": "no source kickoff/release shifting; disputed rows quarantined",
            "results": "no official/on-field score substitution; undefined conflict quarantined",
            "cutoff_gate": "FULL/FAST may run only strictly before first quarantined fixture boundary",
            "model_input_semantics_changed": False,
        },
    }
    return labels, {"adapter_schema": SCHEMA, **source_meta, **audit}, audit


def strict_load_xg_labels(history: list[rt.HistoryFixture], understat_db: Path, confirmation_dir: Path):
    global _AUDIT
    labels, source, audit = _audit_and_labels(history, understat_db, confirmation_dir)
    _AUDIT = audit
    return labels, source


def _ensure_audit(repo_root: Path | None, understat_db: Path | None, confirmation_dir: Path | None) -> None:
    if _AUDIT:
        return
    if repo_root is None or understat_db is None or confirmation_dir is None:
        return
    history, _ = rt.load_frozen_v1_history(repo_root)
    strict_load_xg_labels(history, understat_db, confirmation_dir)


def _guard_cutoff(cutoff: datetime) -> None:
    if not _AUDIT:
        return
    boundary = rt._parse_dt(str(_AUDIT["verified_complete_before_exclusive"]), "source contract safe boundary")
    if cutoff.astimezone(timezone.utc) >= boundary:
        gap = {k: _AUDIT[k] for k in (
            "schema_version", "status", "expected_join_n", "safe_join_n", "time_semantic_gap_n",
            "result_conflict_n", "verified_complete_before_exclusive", "time_semantic_gaps", "result_conflicts", "policy")}
        raise rt.RuntimeGateError("FORMAL_SOURCE_CONTRACT_GAP_AS_OF_CUTOFF " + json.dumps(gap, ensure_ascii=False, sort_keys=True, separators=(",", ":")))


def strict_build_production_state_at_cutoff(repo_root: Path, understat_db: Path, confirmation_dir: Path, cutoff: datetime):
    _ensure_audit(repo_root, understat_db, confirmation_dir)
    _guard_cutoff(cutoff)
    return _ORIGINAL_BUILD(repo_root, understat_db, confirmation_dir, cutoff)


def strict_resolve_state_for_cutoff(bundle_dir, inp, fixture, cutoff, repo_root=None, understat_db=None, confirmation_dir=None,
                                    engineering_history=None, engineering_xg_labels=None, engineering_source=None, engineering_identity=None):
    if engineering_history is None:
        _ensure_audit(repo_root, understat_db, confirmation_dir)
        _guard_cutoff(cutoff)
    return _ORIGINAL_RESOLVE(bundle_dir, inp, fixture, cutoff, repo_root, understat_db, confirmation_dir,
                             engineering_history, engineering_xg_labels, engineering_source, engineering_identity)


def audit_snapshot() -> dict[str, Any]:
    return json.loads(json.dumps(_AUDIT, ensure_ascii=False)) if _AUDIT else {"schema_version": SCHEMA, "status": "NOT_YET_EVALUATED"}


def install() -> dict[str, Any]:
    rt.load_xg_labels = strict_load_xg_labels
    rt.build_production_state_at_cutoff = strict_build_production_state_at_cutoff
    rt.resolve_state_for_cutoff = strict_resolve_state_for_cutoff
    return {
        "schema_version": SCHEMA, "mode": "SAFE_PREFIX_STRICT_FAIL_CLOSED",
        "scientific_modules_changed": False, "history_time_mutation": False,
        "result_substitution": False, "xg_release_mutation": False,
        "team_alias_mapping": "identity-only same-date schedule cooccurrence",
        "post_gap_execution": "FORBIDDEN_UNTIL_GOVERNED_SOURCE_CONTRACT_RESOLUTION",
    }
