...
class N9Error(RuntimeError): pass
def require(c,m):
    if not c: raise N9Error(m)
...
def align_baseline(rows,base):
    sids=[r['fixture_id'] for r in rows]; bids=[r['n2_fixture_id'] for r in base]; require(len(bids)==len(set(bids)),'BASE_DUP'); require(set(sids)==set(bids),'BASE_ID_SET')
...
def state_features(hist,route,w=5):
    if not hist: return None
    xs=hist[-w:]; last=xs[-1]
    if route=='R1_LAST_SHAPE': return list(last['vector'])
    means=[sum(x['vector'][j] for x in xs)/len(xs) for j in range(8)]
    if route=='R2_W5_SHAPE_MEAN': return means
    switches=sum(xs[i]['signature']!=xs[i-1]['signature'] for i in range(1,len(xs)))/max(1,len(xs)-1)
    uniq=len({x['signature'] for x in xs})/len(xs)
    if route in {'R3_W5_STABILITY','R4_MATCHUP_INTERACTION'}: return means+[switches,uniq]
    raise N9Error('ROUTE')

def pair_features(h,a,route):
    d=8 if route in {'R1_LAST_SHAPE','R2_W5_SHAPE_MEAN'} else 10
    hv=[None]*d if h is None else h; av=[None]*d if a is None else a
    base=hv+av+[(None if hv[j] is None or av[j] is None else hv[j]-av[j]) for j in range(d)]
    if route=='R4_MATCHUP_INTERACTION':
        inter=[
            None if hv[0] is None or av[4] is None else hv[0]*av[4],
            None if av[0] is None or hv[4] is None else av[0]*hv[4],
            None if hv[5] is None or av[5] is None else hv[5]*av[5],
            None if hv[6] is None or av[7] is None else hv[6]*av[7],
            None if av[6] is None or hv[7] is None else av[6]*hv[7]
        ]
        return base+inter
    return base

def raw_by_route(rows,route,w=5):
    hist=defaultdict(list); pending=deque(); out=[]
    for s,e in groups(rows):
        ...
        while pending and dt(pending[0]['release_at'])<=ko:
            ...
            for side in ('home','away'):
                sh=r[side+'_shape']
                if sh.get('available') is True:
                    hist[r[side+'_team_id']].append({'vector':[float(x) for x in sh['vector']], 'signature':str(sh['signature'])})
        for i in range(s,e):
            r=rows[i]
            out.append(pair_features(state_features(hist[r['home_team_id']],route,w),state_features(hist[r['away_team_id']],route,w),route))
...
def fit(X,bases,y,c=.25,iters=300):
    Xn=np.asarray([[1.0]+list(map(float,x)) for x in X],dtype=float)
    bn=np.asarray(bases,dtype=float); yn=np.asarray(y,dtype=int); eps=1e-15
    ...
    def objgrad(B):
        ...
        g=((pr[:,:2]-Y).T@Xn)/len(Xn); g[:,1:]+=reg*B[:,1:]
        return loss,g
    loss,g=objgrad(beta)
    for _ in range(iters):
        ...
        while step>1e-8:
            ...
            if nl<=loss-1e-4*step*n2:
                beta,loss,g=cand,nl,ng; ok=True; break
            step*=0.5
...
def blocks(rows,warm=.2,nblocks=5):
    ...
    for k,g in enumerate(rem):
        ...
        if len(out)<nblocks-1 and a>=each and left>=nblocks-len(out)-1:
            out.append((s,g[1])); s=g[1]; a=0
...
def run(prereg,source,baseline,labels,out):
    p=json.loads(Path(prereg).read_text()); rows=readl(source); base=readl(baseline); lab=readl(labels); n=p['data']['development_n']
    require(len(rows)==len(base)==len(lab)==n,'N'); require([r['fixture_id'] for r in rows]==[r['fixture_id'] for r in lab],'LABEL_ALIGN')
    ...
    for sp in p['subroutes']:
        rid=sp['id']; raw=raw_by_route(rows,rid,p['development_protocol']['history_window']); idx=[]; cp=[]
        for s,e in bs:
    ...
    for lg in p['metrics']['group_report']:
        if lg in {'J1','K1'}:
            lr[lg]={'status':'NOT_AVAILABLE','n':0,'coverage':0.,'weight':0,'matrix_delta':0}; continue
    ...
    res={'schema_version':'football3-nova-n9-tactical-shape-development-oof-v1','status':'N9_DEVELOPMENT_OOF_COMPLETE','classification':cls,'development_n':n,'oof_evaluation_n':len(idx),'warmup_end':warm,'completed_matches_only':True,'feature_time_rule':'only prior valid formation descriptors with release_at<=target_kickoff; same-kickoff atomic','routes':routes,'best_signal_route':best['route'],'best_formal_minus_candidate_logloss':best['formal_minus_candidate_logloss'],'selected_route':best['route'] if cls=='POSITIVE_SIGNAL' else None,'league_report':lr,'coach_identity_status':'NOT_AVAILABLE','n1_2025_isolated_labels_read':0,'n2_2023_isolated_labels_read':0,'n3_2021_isolated_labels_read':0,'candidate_activation_allowed':False,'promotion_allowed':False,'formal_v2_changed':False,'current_changed':False,'production_changed':False,'candidate_weight':0,'matrix_delta':0,'score_matrix':{'formal_v2_unchanged':True,'candidate_matrix_delta':0,'exact_score_metrics_changed':False}}
