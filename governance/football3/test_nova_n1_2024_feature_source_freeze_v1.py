...
class FeatureSourceFreezeTests(unittest.TestCase):
    def _db(self, *, bad_ppda=False):
        ...
        con.execute("""create table general_game_stats(
            id integer,date text,league text,season integer,team_h text,team_a text,h_id integer,a_id integer,
            h_deep real,a_deep real,h_ppda real,a_ppda real,h_goals integer,a_goals integer,h_xg real,a_xg real
        )""")
        rows = [
            (1,"2024-08-10 12:00:00","EPL",2024,"A","B",10,20,5,4,9 if not bad_ppda else 0,10,3,1,2.1,0.7),
            (2,"2024-08-11 12:00:00","La liga",2024,"C","D",30,40,6,7,8,11,1,1,1.2,1.3),
        ]
