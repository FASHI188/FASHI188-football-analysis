...
EXPECTED=1826
class N9CoachLabelError(RuntimeError): pass
def require(c,m):
    if not c: raise N9CoachLabelError(m)
...
def run(db:Path,projection:Path,out:Path):
    ...
    receipt={"schema_version":"football3-nova-n9-coach-history-label-vault-v1","status":"N9_COACH_HISTORY_DEVELOPMENT_2022_LABEL_VAULT_PASS","development_n":EXPECTED,"development_season":2022,"queried_season_predicate":2022,"n1_2025_isolated_labels_read":0,"n2_2023_isolated_labels_read":0,"n3_2021_isolated_labels_read":0,"label_sha256":sha256(lp.read_bytes()),"binding_sha256":sha256(canon(binds)),"score_fields_emitted":False,"raw_goals_emitted":False,"training_performed":False,"formal_v2_changed":False,"current_changed":False,"production_changed":False,"candidate_weight":0,"matrix_delta":0}; (out/"label_receipt.json").write_text(json.dumps(receipt,indent=2,sort_keys=True)+"\n"); return receipt
def main():
