...
class N8Error(RuntimeError): pass
def require(c:bool,m:str)->None:
    if not c: raise N8Error(m)
...
def align_baseline(source_rows, baseline_rows):
    ...
    require(len(sids)==len(set(sids)),"SOURCE_DUP_FIXTURE_ID"); require(len(bids)==len(set(bids)),"BASELINE_DUP_FIXTURE_ID"); require(set(sids)==set(bids),f"BASELINE_ID_SET_MISMATCH:missing={len(set(sids)-set(bids))}:extra={len(set(bids)-set(sids))}")
    by={str(r["n2_fixture_id"]):r for r in baseline_rows}; out=[by[x] for x in sids]
    for s,b in zip(source_rows,out):
        require(dt(str(s["kickoff"]))==dt(str(b["kickoff"])),f"KICKOFF_MISMATCH:{s['fixture_id']}")
        require(str(s["home_team_id"])==str(b["home_team_id"]),f"HOME_MISMATCH:{s['fixture_id']}")
        require(str(s["away_team_id"])==str(b["away_team_id"]),f"AWAY_MISMATCH:{s['fixture_id']}")
        require(str(s["league"])==str(b["league"]),f"LEAGUE_MISMATCH:{s['fixture_id']}")
...
def team_match_for(side): return [float(side["openplay_xg_per_shot"]),float(side["setpiece_xg_share"]),float(side["highq_np_shot_share"]),float(side["np_shot_xg_sd"])]
def state(hist,w=10):
    if not hist: return [None]*8+[0.0]
    xs=hist[-w:]; n=len(xs); vals=[]
    for j in range(8): vals.append(sum(float(x[j]) for x in xs)/n)
    return vals+[float(len(hist))]

def raw_features(rows,w=10):
    ...
    for s,e in groups(rows):
        ...
        while pending and dt(pending[0]["release_at"])<=ko:
            r=pending.popleft(); hf=team_match_for(r["home"]); af=team_match_for(r["away"])
            h[r["home_team_id"]].append(hf+af); h[r["away_team_id"]].append(af+hf)
        for i in range(s,e):
            r=rows[i]; out[i]=state(h[r["home_team_id"]],w)+state(h[r["away_team_id"]],w)
        for i in range(s,e): pending.append(rows[i])
    return out

def primitive(row,route):
    H=row[:9]; A=row[9:18]; hn=math.log1p(H[8]); an=math.log1p(A[8])
    r1=[H[0],H[4],A[0],A[4],hn,an]
    r2=[H[1],H[5],A[1],A[5],hn,an]
    r3=[H[2],H[6],H[3],H[7],A[2],A[6],A[3],A[7],hn,an]
    if route=="R1_OPENPLAY_QUALITY": return r1
    if route=="R2_SETPIECE_SHARE": return r2
    if route=="R3_HIGHQ_DISPERSION": return r3
    if route=="R4_COMBINED_FIXED": return r1[:-2]+r2[:-2]+r3
    raise N8Error("UNKNOWN_ROUTE")

def scaler(X,idx):
    means=[]; stds=[]
    for j in range(len(X[0])):
        vals=[float(X[i][j]) for i in idx if X[i][j] is not None and math.isfinite(float(X[i][j]))]
        m=sum(vals)/len(vals) if vals else 0.0; var=sum((v-m)**2 for v in vals)/len(vals) if vals else 0.0
        means.append(m); stds.append(max(math.sqrt(var),1e-9))
    return means,stds
...
def softmax(base,x,beta):
    ...
    lh=oh+sum(beta[0][j]*xx[j] for j in range(len(xx))); ld=od+sum(beta[1][j]*xx[j] for j in range(len(xx))); m=max(lh,ld,0.0)
    eh,ed,ea=math.exp(lh-m),math.exp(ld-m),math.exp(-m); t=eh+ed+ea; return [eh/t,ed/t,ea/t]
...
def run(prereg:Path,source:Path,baseline:Path,labels:Path,out:Path):
    ...
    require(len(rows)==len(base_rows)==len(lab)==n,"ROW_N")
    ...
    raw=raw_features(rows,int(p["model"]["state_window"])); warm,bs=blocks(rows,float(p["development_protocol"]["warmup_fraction"]),int(p["development_protocol"]["oof_blocks"]))
    routes=[]; pred_by={}; oof_ref=None
    for spec in p["subroutes"]:
        rid=spec["id"]; prim=[primitive(r,rid) for r in raw]; idx=[]; cand=[]
        for fs,fe in bs:
            tr=list(range(fs)); va=list(range(fs,fe)); m,s=scaler(prim,tr)
            Xtr=[zrow(prim[i],m,s) for i in tr]; Xva=[zrow(prim[i],m,s) for i in va]
            beta,_,_=fit(Xtr,[probs[i] for i in tr],[y[i] for i in tr],float(p["model"]["ridge_c"]))
            cand.extend(softmax(probs[i],x,beta) for i,x in zip(va,Xva)); idx.extend(va)
    ...
    best=max(routes,key=lambda r:r["formal_minus_candidate_logloss"])
    classification="POSITIVE_SIGNAL" if any(r["qualified"] or r["positive_floor"] for r in routes) else "FAIL_RESEARCH_DIRECTION"
    selected=best["route"] if classification=="POSITIVE_SIGNAL" else None
    ...
    for league in p["metrics"]["group_report"]:
        if league in {"J1","K1"}:
            league_report[league]={"status":"NOT_AVAILABLE","n":0,"coverage":0.0,"weight":0,"matrix_delta":0}; continue
        pos=[j for j,i in enumerate(idx) if rows[i]["league"]==league]; lb=[b[j] for j in pos]; lc=[chosen[j] for j in pos]; ly=[yy[j] for j in pos]
        bm=metrics(lb,ly); cm=metrics(lc,ly)
    ...
    result={"schema_version":"football3-nova-n8-shot-quality-setpiece-development-oof-v1","status":"N8_DEVELOPMENT_OOF_COMPLETE","classification":classification,"development_n":n,"oof_evaluation_n":len(idx),"warmup_end":warm,"completed_matches_only":True,"feature_time_rule":"only prior completed match event aggregates with release_at<=target_kickoff; same-kickoff atomic","routes":routes,"best_signal_route":best["route"],"best_formal_minus_candidate_logloss":best["formal_minus_candidate_logloss"],"selected_route":selected,"league_report":league_report,"n1_2025_isolated_labels_read":0,"n2_2023_isolated_labels_read":0,"n3_2021_isolated_labels_read":0,"candidate_activation_allowed":False,"promotion_allowed":False,"formal_v2_changed":False,"current_changed":False,"production_changed":False,"candidate_weight":0,"matrix_delta":0,"score_matrix":{"formal_v2_unchanged":True,"candidate_matrix_delta":0,"exact_score_metrics_changed":False}}
