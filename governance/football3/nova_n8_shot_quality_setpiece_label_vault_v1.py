...
EXPECTED=1826
class N8LabelError(RuntimeError): pass
def require(c,m):
    if not c: raise N8LabelError(m)
...
def run(db:Path,projection:Path,out:Path):
    ...
    conn=sqlite3.connect(f"file:{db}?mode=ro",uri=True); rows=conn.execute("SELECT date,h_id,a_id,league,h_goals,a_goals FROM general_game_stats WHERE season=2022 AND league IN ('EPL','Bundesliga','La liga','La_liga','Ligue 1','Ligue_1','Serie A','Serie_A') ORDER BY date,h_id,a_id").fetchall(); conn.close(); require(len(rows)==EXPECTED,f"DB_N:{len(rows)}")
    idx={}
    for r in rows:
    ...
    rec={"schema_version":"football3-nova-n8-label-vault-v1","status":"N8_DEVELOPMENT_2022_LABEL_VAULT_PASS","development_n":EXPECTED,"development_season":2022,"queried_season_predicate":2022,"n1_2025_isolated_labels_read":0,"n2_2023_isolated_labels_read":0,"n3_2021_isolated_labels_read":0,"label_sha256":sha256(lp.read_bytes()),"binding_sha256":sha256(canon(binds)),"score_fields_emitted":False,"raw_goals_emitted":False,"training_performed":False,"formal_v2_changed":False,"current_changed":False,"production_changed":False,"candidate_weight":0,"matrix_delta":0}; (out/"label_receipt.json").write_text(json.dumps(rec,indent=2,sort_keys=True)+"\n"); return rec
def main():
