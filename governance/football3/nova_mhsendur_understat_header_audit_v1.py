...
def git_blob_sha1(data: bytes) -> str:
    header=f"blob {len(data)}\0".encode("ascii"); return hashlib.sha1(header+data).hexdigest()
def validate_lock(lock: dict) -> None:
    if lock.get("schema_version") != "football3-nova-n1-mhsendur-understat-header-lock-v1": raise AuditError("unexpected schema_version")
    src=lock.get("source",{})
    for key in ("repository","revision","readme_blob_sha1","archive_path","archive_blob_sha1","raw_url"):
        if not src.get(key): raise AuditError(f"missing source field {key}")
    perm=lock.get("permission",{})
    if perm.get("class") != "RESEARCH_ONLY_EXPLICIT_REUSE": raise AuditError("permission class drift")
    if perm.get("production_eligible") is not False: raise AuditError("research-only source cannot be production eligible")
    gov=lock.get("governance",{})
    if gov.get("candidate_confirmation_allowed") is not False: raise AuditError("historical source cannot be candidate confirmation")
    if any(gov.get(k) is not False for k in ("formal_v2_changed","current_changed","production_changed")): raise AuditError("formal state change forbidden")
    roles=lock.get("locked_feature_roles"); expected=["date","h_a","deep","deep_allowed","ppda","ppda_allowed"]
    if roles != expected: raise AuditError("locked feature roles drift")
def fetch_bytes(url:str)->bytes:
    req=urllib.request.Request(url,headers={"User-Agent":"Football3-Nova-N1-Header-Audit/1.0"})
    with urllib.request.urlopen(req,timeout=60) as r: return r.read()
def decode_header_line(line: bytes) -> tuple[str, str]:
    for encoding in ("utf-8-sig", "cp1252", "latin-1"):
        try:
            return line.decode(encoding).rstrip("\r\n"), encoding
        except UnicodeDecodeError:
            continue
    raise AuditError("unable to decode CSV header")

def header_fields(zf:zipfile.ZipFile,member:str)->tuple[list[str], str]:
    with zf.open(member,"r") as fh: line=fh.readline(131072)
    if not line: raise AuditError(f"empty CSV member {member}")
    text, encoding = decode_header_line(line)
    return next(csv.reader([text])), encoding
def audit(lock:dict,archive:bytes)->dict:
    validate_lock(lock); expected_blob=lock["source"]["archive_blob_sha1"]; observed_blob=git_blob_sha1(archive)
