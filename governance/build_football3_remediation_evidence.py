...
def main():
 ...
 changed=git(root,'diff','--name-only',a.base_sha,head).splitlines();fm=[]
 for rel in sorted(set(changed)):
  p=root/rel;fm.append({'path':rel,'size':p.stat().st_size,'sha256':sha(p)} if p.is_file() else {'path':rel,'deleted':True})
